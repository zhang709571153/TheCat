...
$batchSlug = "batch_100_dream_theme_scene_tokens_imagegen_2026-06-25"
$batchDirRelative = "design/development/asset_candidates/ui/dream_theme_scene_tokens/$batchSlug"
$batchDir = Join-Path $projectRoot ($batchDirRelative -replace "/", [System.IO.Path]::DirectorySeparatorChar)

$sourceRelative = "$batchDirRelative/source/thecat_ui_dream_theme_scene_tokens_batch100_chromakey_source_v001.png"
$alphaRelative = "$batchDirRelative/source/thecat_ui_dream_theme_scene_tokens_batch100_alpha_sheet_v001.png"
$namesRelative = "$batchDirRelative/source/thecat_ui_theme_token_batch100_names.txt"
$assetTableRelative = "$batchDirRelative/thecat_ui_dream_theme_scene_tokens_batch100_asset_table.csv"
$manifestRelative = "$batchDirRelative/thecat_ui_theme_token_batch100_semantic_manifest.csv"
$contactSheetRelative = "$batchDirRelative/thecat_ui_theme_token_batch100_semantic_contact_sheet_v001.png"
$readabilityBoardRelative = "$batchDirRelative/thecat_ui_theme_token_batch100_64px_theme_selection_board_v001.png"
$finalReviewRelative = "$batchDirRelative/thecat_ui_theme_token_batch100_final_review.csv"
$processNoteRelative = "$batchDirRelative/thecat_ui_theme_token_batch100_process_note.md"
$reviewNoteRelative = "$batchDirRelative/reviews/BATCH100_DREAM_THEME_SCENE_TOKENS_AGENT_REVIEW_2026-06-25.md"
$mirroredReviewNoteRelative = "design/development/asset_review/BATCH100_DREAM_THEME_SCENE_TOKENS_AGENT_REVIEW_2026-06-25.md"

$expectedSemantics = @(
    "dream_theme_bedroom_gate_token",
    "dream_theme_egypt_gate_token",
    "dream_theme_locked_theme_seal",
    "dream_theme_available_theme_glow",
    "dream_theme_current_theme_crown",
    "dream_theme_unknown_theme_veil",
    "dream_theme_boss_domain_warning",
...
function Test-RequiredFile {
    ...
    if (-not (Test-Path -LiteralPath $path)) {
        Add-Failure "Missing required Batch 100 file: $RelativePath"
        return $null
    }

    if ($path.Length -ge 260) {
        Add-Failure "Path exceeds legacy Windows 260-character boundary: $RelativePath"
    }
...
if (-not (Test-Path -LiteralPath $batchDir)) {
    throw "Batch 100 directory is missing: $batchDirRelative"
}
...
foreach ($semantic in $expectedSemantics) {
    ...
    if ($row.source_truth -match "IAd|MDr|IZp|HDo|FoW9|character|Batch65|Batch 65|map archive") {
    ...
    if ($row.target_use -ne "candidate_only_theme_selection_import_test") {
...
foreach ($semantic in $expectedSemantics) {
    ...
    if ($row.batch -ne "batch100") {
        Add-Failure "$semantic manifest batch should be batch100"
    }
    ...
    if ($candidatePath.Length -ge 260) {
        Add-Failure "$semantic candidate PNG path exceeds legacy Windows 260-character boundary"
    }
...
foreach ($semantic in $expectedSemantics) {
    ...
    if ($row.unity_gate -ne "theme_selection_screenshot; import_settings; binding_proof; clean_console") {
    ...
    if ($row.collider_policy -ne "none_ui_hit_target_owned_by_theme_slot") {
...
if ($metaFiles.Count -gt 0) {
    Add-Failure "Batch 100 candidate directory must not contain Unity .meta files"
}

$assetFiles = @(
    Get-ChildItem -LiteralPath (Join-Path $projectRoot "Assets") -Recurse -File -ErrorAction SilentlyContinue |
        Where-Object {
            $_.Name -like "*theme_token*batch100*" -or
            $_.Name -like "*dream_theme*batch100*" -or
            $_.FullName -like "*dream_theme_scene_tokens*batch_100*"
        }
)
if ($assetFiles.Count -gt 0) {
    Add-Failure "Batch 100 must not write runtime Assets files before approval"
}
...
if ($null -ne $processPath -and (Test-Path -LiteralPath $processPath)) {
    ...
    foreach ($token in @("built-in Codex imagegen", "magenta chroma-key", "candidate_only_pending_unity_theme_selection_review", "No Unity import was performed", "HDo map art archive is still ACL-blocked")) {
...
if ($null -ne $reviewPath -and $null -ne $mirroredPath -and (Test-Path -LiteralPath $reviewPath) -and (Test-Path -LiteralPath $mirroredPath)) {
    ...
    if ($mirroredReviewText -ne $reviewText) {
        Add-Failure "Mirrored Batch 100 review note differs from candidate-package review note"
    }
}

if ($failures.Count -gt 0) {
    Write-Error ("Batch 100 dream-theme scene token validation failed:`n" + ($failures -join "`n"))
    exit 1
}

Write-Output "Batch 100 dream-theme scene token validation passed. Rows: $($manifestRows.Count)"
