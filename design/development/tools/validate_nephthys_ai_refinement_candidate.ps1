...
$manifestRelative = "design/development/asset_candidates/starter_cats/batch_36_nephthys_ai_refinement_candidate_2026-06-15/nephthys_batch36_ai_refinement_manifest.csv"
...
if (-not (Test-Path $manifestPath)) {
    throw "Nephthys AI refinement manifest is missing: $manifestRelative"
}

$rows = @(Import-Csv -LiteralPath $manifestPath)
if ($rows.Count -ne $expectedTypes.Count) {
    Add-Failure "Expected $($expectedTypes.Count) Nephthys AI refinement rows but found $($rows.Count)"
}
...
foreach ($row in $rows) {
    ...
    if ($row.candidate_path -notlike "design/development/asset_candidates/starter_cats/nephthys/batch_36_nephthys_ai_refinement_candidate_2026-06-15/*") {
        Add-Failure "$($row.candidate_path) must stay in the Nephthys Batch 36 candidate directory"
    }
...
if ($reviewNotesSeen.Count -ne 1) {
    Add-Failure "Expected one Nephthys AI review note but found $($reviewNotesSeen.Count)"
}

if ($reviewSheetsSeen.Count -ne 1) {
    Add-Failure "Expected one Nephthys AI review sheet but found $($reviewSheetsSeen.Count)"
}

if ($promptRecordsSeen.Count -ne 1) {
    Add-Failure "Expected one Nephthys AI prompt record but found $($promptRecordsSeen.Count)"
}
...
foreach ($promptRelative in $promptRecordsSeen) {
    ...
    foreach ($token in @("gold-brown tabby", "golden eyes", "deep navy hood", "crescent", "blue tear gem", "sand-gold", "ankh", "floating pyramid", "striped tail", "Avoid")) {
        if ($content.IndexOf($token, [System.StringComparison]::OrdinalIgnoreCase) -lt 0) {
...
if ($failures.Count -gt 0) {
    ...
    throw "Nephthys AI refinement validation failed with $($failures.Count) issue(s)."
}

Write-Host "Nephthys AI refinement candidate validation passed."
