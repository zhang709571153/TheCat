...
$batchSlug = "batch_88_character_select_preflight_2026-06-25"
$batchDirRelative = "design/development/asset_candidates/ui/character_select/$batchSlug"
$batchDir = Join-Path $projectRoot ($batchDirRelative -replace "/", [System.IO.Path]::DirectorySeparatorChar)
$manifestRelative = "$batchDirRelative/thecat_ui_character_select_batch88_manifest.csv"
$manifestPath = Join-Path $projectRoot ($manifestRelative -replace "/", [System.IO.Path]::DirectorySeparatorChar)
$contactSheetRelative = "$batchDirRelative/thecat_ui_character_select_batch88_contact_sheet_v001.png"
$reviewSheetRelative = "$batchDirRelative/thecat_ui_character_select_batch88_review_sheet_v001.png"
$reviewNoteRelative = "$batchDirRelative/thecat_ui_character_select_batch88_candidate_review.md"
$processNoteRelative = "$batchDirRelative/thecat_ui_character_select_batch88_process_note.md"
$agentPromptRelative = "$batchDirRelative/thecat_ui_character_select_batch88_agent_review_prompt.md"

$expectedVariants = [ordered]@{
    character_select_stage_panel = "1180x640"
    starter_card_frame_selected = "360x500"
    starter_card_frame_idle = "360x500"
    starter_role_chip_strip = "360x96"
    starter_ready_badge = "220x80"
    starter_launch_button_frame = "420x112"
    character_select_1920x1080 = "1920x1080"
    character_select_1365x768 = "1365x768"
    character_select_1280x720 = "1280x720"
    character_select_1024x768 = "1024x768"
}

$expectedTypes = @{
    character_select_stage_panel = "sprite"
    starter_card_frame_selected = "sprite"
    starter_card_frame_idle = "sprite"
    starter_role_chip_strip = "sprite"
    starter_ready_badge = "sprite"
    starter_launch_button_frame = "sprite"
    character_select_1920x1080 = "local_mockup"
    character_select_1365x768 = "local_mockup"
    character_select_1280x720 = "local_mockup"
    character_select_1024x768 = "local_mockup"
}
...
if (-not (Test-Path -LiteralPath $manifestPath)) {
    throw "Batch 88 manifest is missing: $manifestRelative"
}

$rows = @(Import-Csv -LiteralPath $manifestPath)
if ($rows.Count -ne 10) {
    Add-Failure "Expected 10 Batch 88 rows but found $($rows.Count)"
}
...
foreach ($variant in $expectedVariants.Keys) {
    ...
    if ($row.agent_prompt -ne $agentPromptRelative) {
        Add-Failure "$variant agent_prompt should be $agentPromptRelative but found $($row.agent_prompt)"
    }

    $agentPromptPath = Resolve-ProjectPath $agentPromptRelative
    ...
    if ($row.source_model -ne "deterministic_local_derivative_from_existing_ui_and_source_locked_hud_assets_not_image2") {
        Add-Failure "$variant must not claim image2 provenance. source_model=$($row.source_model)"
    }

    if ($row.recommendation -ne "candidate_only_pending_unity_character_select_screenshots") {
        Add-Failure "$variant recommendation mismatch: $($row.recommendation)"
    }

    if ($row.candidate_path -notlike "$batchDirRelative/*") {
        Add-Failure "$variant candidate_path must stay in Batch 88 directory: $($row.candidate_path)"
    }
...
foreach ($png in $actualPngs) {
    ...
    if ($png -notin $manifestPaths) {
        Add-Failure "Found orphan Batch 88 PNG not listed in manifest: $png"
    }
}

foreach ($sharedPath in @($contactSheetRelative, $reviewSheetRelative, $reviewNoteRelative, $processNoteRelative, $agentPromptRelative)) {
    $full = Resolve-ProjectPath $sharedPath
    if ($null -ne $full -and -not (Test-Path -LiteralPath $full)) {
        Add-Failure "Missing shared Batch 88 evidence: $sharedPath"
    }
}
...
if ($null -ne $processPath -and (Test-Path -LiteralPath $processPath)) {
    ...
    foreach ($token in @("ui_character_select", "Qr1", "IAd", "Feishu", "ACL-blocked", "OPENAI_API_KEY", "model selector", "source-locked HUD avatars", "no baked Chinese text", "not image2 provenance", "Unity-rendered character-select screenshots")) {
...
if ($null -ne $reviewPath -and (Test-Path -LiteralPath $reviewPath)) {
    ...
    foreach ($token in @("character-select", "three starter cards", "selected/idle", "1024x768", "Click-target proof")) {
        if ($reviewText -notlike "*$token*") {
            Add-Failure "Review note missing expected Batch 88 token: $token"
        }
    }
}
...
if ($metaFiles.Count -gt 0) {
    Add-Failure "Batch 88 candidate directory must not contain Unity .meta files"
}

if ($failures.Count -gt 0) {
    Write-Error ("Batch 88 character-select preflight validation failed:`n" + ($failures -join "`n"))
    exit 1
}

Write-Output "Batch 88 character-select preflight validation passed. Rows: $($rows.Count)"
