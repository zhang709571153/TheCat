...
def main() -> int:
    ...
    for index, (cat, skill) in enumerate(ICONS):
        ...
        icon_256 = fit_icon(source.crop(box), 256, 232)
        contact.alpha_composite(icon_256, ((index % 6) * 256, (index // 6) * 256))
        ...
        for size, max_fill in ((256, 232), (128, 116), (64, 58), (32, 29)):
            ...
            directory = args.out_dir / f"icons_lightframe_{size}"
            directory.mkdir(parents=True, exist_ok=True)
            filename = f"{asset_id_base}_{size}_candidate_v003_lightframe.png"
            output_path = directory / filename
            output.save(output_path)
            rows.append(
                {
                    "asset_id": f"{asset_id_base}_{size}_candidate_v003_lightframe",
                    "cat": cat,
                    "skill": skill,
                    "size": f"{size}x{size}",
                    "path": output_path.as_posix(),
                    "status": "candidate_lightframe_pending_visual_review",
                    "source": args.input.as_posix(),
                }
            )

    contact.save(args.out_dir / "thecat_ui_starter_skill_icon_motifs_batch80_v003_lightframe_contact_sheet.png")
