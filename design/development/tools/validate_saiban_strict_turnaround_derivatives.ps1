...
$manifestRelative = "design/development/asset_candidates/starter_cats/batch_29_strict_turnaround_derivatives_2026-06-14/saiban_batch29_strict_turnaround_manifest.csv"
...
if (-not (Test-Path $manifestPath)) {
    throw "Saiban strict-turnaround manifest is missing: $manifestRelative"
}

$rows = @(Import-Csv -LiteralPath $manifestPath)
if ($rows.Count -ne $expectedTypes.Count) {
    Add-Failure "Expected $($expectedTypes.Count) Saiban candidate rows but found $($rows.Count)"
}
...
foreach ($row in $rows) {
    ...
    if ($row.candidate_path -notlike "design/development/asset_candidates/starter_cats/saiban/batch_29_strict_turnaround_derivatives_2026-06-14/*") {
        Add-Failure "$($row.candidate_path) must stay in the Saiban Batch 29 candidate directory"
    }
...
if ($reviewNotesSeen.Count -ne 1) {
    Add-Failure "Expected one Saiban review note but found $($reviewNotesSeen.Count)"
}

if ($reviewSheetsSeen.Count -ne 1) {
    Add-Failure "Expected one Saiban review sheet but found $($reviewSheetsSeen.Count)"
}
...
if ($failures.Count -gt 0) {
    ...
    throw "Saiban strict-turnaround derivative validation failed with $($failures.Count) issue(s)."
}

Write-Host "Saiban strict-turnaround derivative validation passed."
