...
$batchSlug = "batch_84_result_settlement_preflight_2026-06-25"
$batchDirRelative = "design/development/asset_candidates/ui/result_settlement/$batchSlug"
$batchDir = Join-Path $projectRoot ($batchDirRelative -replace "/", [System.IO.Path]::DirectorySeparatorChar)
$manifestRelative = "$batchDirRelative/thecat_ui_result_settlement_batch84_manifest.csv"
$manifestPath = Join-Path $projectRoot ($manifestRelative -replace "/", [System.IO.Path]::DirectorySeparatorChar)
$contactSheetRelative = "$batchDirRelative/thecat_ui_result_settlement_batch84_contact_sheet_v001.png"
$reviewSheetRelative = "$batchDirRelative/thecat_ui_result_settlement_batch84_review_sheet_v001.png"
$reviewNoteRelative = "$batchDirRelative/thecat_ui_result_settlement_batch84_candidate_review.md"
$processNoteRelative = "$batchDirRelative/thecat_ui_result_settlement_batch84_process_note.md"
$specRelative = "$batchDirRelative/thecat_ui_result_settlement_batch84_agent_review_prompt.md"

$expectedVariants = [ordered]@{
    panel_frame = "960x540"
    reward_row_frame = "760x96"
    stat_chip_frame = "256x72"
    action_button_frame = "384x96"
    outcome_divider = "640x32"
    success_stamp_ring = "256x256"
    failure_stamp_ring = "256x256"
    battle_victory_1920x1080 = "1920x1080"
    battle_defeat_1920x1080 = "1920x1080"
    run_cleared_1365x768 = "1365x768"
    run_failed_1024x768 = "1024x768"
}

$expectedTypes = @{
    panel_frame = "sprite"
    reward_row_frame = "sprite"
    stat_chip_frame = "sprite"
    action_button_frame = "sprite"
    outcome_divider = "sprite"
    success_stamp_ring = "sprite"
    failure_stamp_ring = "sprite"
    battle_victory_1920x1080 = "local_mockup"
    battle_defeat_1920x1080 = "local_mockup"
    run_cleared_1365x768 = "local_mockup"
    run_failed_1024x768 = "local_mockup"
}
...
$sourcePaths = @(
    ...
    "Assets/TheCat/Art/UI/Banners/thecat_ui_battle_result_victory_banner_512x160_v001.png",
    "Assets/TheCat/Art/UI/Banners/thecat_ui_battle_result_defeat_banner_512x160_v001.png",
    "Assets/TheCat/Art/UI/Banners/thecat_ui_settlement_run_cleared_banner_512x160_v001.png",
    "Assets/TheCat/Art/UI/Banners/thecat_ui_settlement_run_failed_banner_512x160_v001.png",
    "Assets/TheCat/Art/UI/Icons/thecat_ui_reward_dreamshard_icon_128_v001.png",
...
if (-not (Test-Path -LiteralPath $manifestPath)) {
    throw "Batch 84 manifest is missing: $manifestRelative"
}

$rows = @(Import-Csv -LiteralPath $manifestPath)
if ($rows.Count -ne 11) {
    Add-Failure "Expected 11 Batch 84 rows but found $($rows.Count)"
}
...
foreach ($variant in $expectedVariants.Keys) {
    ...
    if ($row.source_model -ne "deterministic_local_derivative_not_image2") {
        Add-Failure "$variant must not claim image2 provenance. source_model=$($row.source_model)"
    }

    if ($row.recommendation -ne "candidate_only_pending_unity_result_settlement_screenshots") {
        Add-Failure "$variant recommendation mismatch: $($row.recommendation)"
    }

    if ($row.candidate_path -notlike "$batchDirRelative/*") {
        Add-Failure "$variant candidate_path must stay in Batch 84 directory: $($row.candidate_path)"
    }
...
foreach ($png in $actualPngs) {
    ...
    if ($png -notin $manifestPaths) {
        Add-Failure "Found orphan Batch 84 PNG not listed in manifest: $png"
    }
}
...
foreach ($sharedPath in @($contactSheetRelative, $reviewSheetRelative, $reviewNoteRelative, $processNoteRelative, $specRelative)) {
    ...
    if ($null -ne $full -and -not (Test-Path -LiteralPath $full)) {
        Add-Failure "Missing shared Batch 84 evidence: $sharedPath"
    }
}
...
if ($null -ne $processPath -and (Test-Path -LiteralPath $processPath)) {
    ...
    foreach ($token in @("result_settlement", "Qr1", "candidate-only", "no baked Chinese text", "not image2 provenance", "Unity-rendered result/settlement screenshots")) {
...
if ($null -ne $reviewPath -and (Test-Path -LiteralPath $reviewPath)) {
    ...
    foreach ($token in @("battle victory", "battle defeat", "run-cleared", "run-failed", "1024x768")) {
        if ($reviewText -notlike "*$token*") {
            Add-Failure "Review note missing expected Batch 84 token: $token"
        }
    }
}
...
if ($metaFiles.Count -gt 0) {
    Add-Failure "Batch 84 candidate directory must not contain Unity .meta files"
}

if ($failures.Count -gt 0) {
    Write-Error ("Batch 84 result/settlement preflight validation failed:`n" + ($failures -join "`n"))
    exit 1
}

Write-Output "Batch 84 result/settlement preflight validation passed. Rows: $($rows.Count)"
