...
$batchSlug = "batch_83_loading_start_preflight_2026-06-25"
$batchDirRelative = "design/development/asset_candidates/ui/loading_start/$batchSlug"
$batchDir = Join-Path $projectRoot ($batchDirRelative -replace "/", [System.IO.Path]::DirectorySeparatorChar)
$manifestRelative = "$batchDirRelative/thecat_ui_loading_start_batch83_manifest.csv"
$manifestPath = Join-Path $projectRoot ($manifestRelative -replace "/", [System.IO.Path]::DirectorySeparatorChar)
$contactSheetRelative = "$batchDirRelative/thecat_ui_loading_start_batch83_contact_sheet_v001.png"
$reviewSheetRelative = "$batchDirRelative/thecat_ui_loading_start_batch83_review_sheet_v001.png"
$reviewNoteRelative = "$batchDirRelative/thecat_ui_loading_start_batch83_candidate_review.md"
$processNoteRelative = "$batchDirRelative/thecat_ui_loading_start_batch83_process_note.md"
$specRelative = "$batchDirRelative/thecat_ui_loading_start_batch83_agent_review_prompt.md"

$expectedVariants = [ordered]@{
    progress_frame = "640x48"
    progress_fill = "640x48"
    spinner_crescent = "128x128"
    dot_sequence = "384x64"
    mockup_1920x1080 = "1920x1080"
    mockup_1365x768 = "1365x768"
    mockup_1280x720 = "1280x720"
    mockup_1024x768 = "1024x768"
}

$expectedTypes = @{
    progress_frame = "sprite"
    progress_fill = "sprite"
    spinner_crescent = "sprite"
    dot_sequence = "sprite"
    mockup_1920x1080 = "local_mockup"
    mockup_1365x768 = "local_mockup"
    mockup_1280x720 = "local_mockup"
    mockup_1024x768 = "local_mockup"
}
...
if (-not (Test-Path -LiteralPath $manifestPath)) {
    throw "Batch 83 manifest is missing: $manifestRelative"
}

$rows = @(Import-Csv -LiteralPath $manifestPath)
if ($rows.Count -ne 8) {
    Add-Failure "Expected 8 Batch 83 rows but found $($rows.Count)"
}
...
foreach ($variant in $expectedVariants.Keys) {
    ...
    if ($row.candidate_path -notlike "$batchDirRelative/*") {
        Add-Failure "$variant candidate_path must stay in Batch 83 directory: $($row.candidate_path)"
    }
...
foreach ($png in $actualPngs) {
    ...
    if ($png -notin $manifestPaths) {
        Add-Failure "Found orphan Batch 83 PNG not listed in manifest: $png"
    }
}
...
foreach ($sharedPath in @($contactSheetRelative, $reviewSheetRelative, $reviewNoteRelative, $processNoteRelative, $specRelative)) {
    ...
    if ($null -ne $full -and -not (Test-Path -LiteralPath $full)) {
        Add-Failure "Missing shared Batch 83 evidence: $sharedPath"
    }
}
...
if ($null -ne $processPath -and (Test-Path -LiteralPath $processPath)) {
    ...
    foreach ($token in @("ui_loading_start", "Qr1 UI/style", "candidate-only", "no baked Chinese text", "not image2 provenance", "Unity-rendered loading/start screenshots")) {
...
if ($metaFiles.Count -gt 0) {
    Add-Failure "Batch 83 candidate directory must not contain Unity .meta files"
}

if ($failures.Count -gt 0) {
    Write-Error ("Batch 83 loading/start preflight validation failed:`n" + ($failures -join "`n"))
    exit 1
}

Write-Output "Batch 83 loading/start preflight validation passed. Rows: $($rows.Count)"
