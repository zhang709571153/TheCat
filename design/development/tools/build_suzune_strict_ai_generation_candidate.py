...
BATCH_SLUG = "batch_51_suzune_strict_ai_generation_candidate_2026-06-15"
CAT_ID = "suzune"
...
MANIFEST_PATH = BATCH_DIR / "suzune_batch51_strict_ai_generation_manifest.csv"
REVIEW_SHEET_PATH = CANDIDATE_DIR / "thecat_cat_suzune_batch51_strict_ai_generation_review_sheet.png"
REVIEW_NOTE_PATH = CANDIDATE_DIR / "suzune_batch51_strict_ai_generation_candidate_review.md"
PROCESS_NOTE_PATH = CANDIDATE_DIR / "suzune_batch51_strict_ai_generation_process_note.md"
AGENT_PROMPT_PATH = REPO_ROOT / "design" / "development" / "agent_prompts" / "p0_asset_batch_51_suzune_strict_ai_generation_candidate.md"

SOURCE_TURNAROUND = "design/\u68a6\u5883\u652f\u914d\u8005\u6838\u5fc3\u73a9\u6cd5/assets/characters/ch03_suzune_sleep_shrine_maiden/turnaround/suzune_turnaround_colored_2026-06-03.png"
BATCH47_JSON = "design/development/asset_candidates/starter_cats/suzune/batch_47_starter_cat_strict_generation_spec_pack_2026-06-15/thecat_cat_suzune_batch47_strict_generation_spec_v001.json"
BATCH47_CARD = "design/development/asset_candidates/starter_cats/suzune/batch_47_starter_cat_strict_generation_spec_pack_2026-06-15/thecat_cat_suzune_batch47_strict_generation_spec_card_v001.png"
BATCH35_ALPHA = "design/development/asset_candidates/starter_cats/suzune/batch_35_suzune_cutout_candidate_2026-06-15/thecat_cat_suzune_cutout_alpha_1024_candidate_v001.png"
BATCH35_REVIEW = "design/development/asset_candidates/starter_cats/suzune/batch_35_suzune_cutout_candidate_2026-06-15/suzune_batch35_cutout_candidate_review.md"
UNITY_SPRITE = "Assets/TheCat/Art/Characters/Sprites/thecat_cat_suzune_combat_sprite_512_v001.png"

CHROMA_SOURCE_NAME = "thecat_cat_suzune_batch51_strict_ai_chromakey_source_v001.png"
ALPHA_NAME = "thecat_cat_suzune_batch51_strict_ai_alpha_1024_candidate_v001.png"
PREVIEW_NAME = "thecat_cat_suzune_batch51_strict_ai_alpha_512_preview_v001.png"
CHECKER_NAME = "thecat_cat_suzune_batch51_strict_ai_checkerboard_512_review_v001.png"
DARK_NAME = "thecat_cat_suzune_batch51_strict_ai_darkfield_512_review_v001.png"
WARM_NAME = "thecat_cat_suzune_batch51_strict_ai_warmfield_512_review_v001.png"
MASK_NAME = "thecat_cat_suzune_batch51_strict_ai_alpha_mask_512_review_v001.png"
...
FIELD_NAMES = (
    ...
    "batch47_card_sha256",
    "batch35_alpha_path",
    "batch35_alpha_sha256",
    "batch35_review_path",
    "batch35_review_sha256",
    "unity_sprite_path",
    "unity_sprite_sha256",
...
def main() -> None:
    ...
    write_manifest(rows)
    print("Wrote Suzune Batch 51 strict AI generation candidate pack.")
...
def build_manifest_rows() -> list[dict[str, str]]:
    ...
    batch35_alpha = resolve_repo_path(BATCH35_ALPHA)
    batch35_review = resolve_repo_path(BATCH35_REVIEW)
        ...
        rows.append(
            {
                ...
                "batch35_alpha_path": BATCH35_ALPHA,
                "batch35_alpha_sha256": sha256(batch35_alpha),
                "batch35_review_path": BATCH35_REVIEW,
                "batch35_review_sha256": sha256(batch35_review),
...
def write_review_sheet() -> None:
    ...
    batch35 = Image.open(resolve_repo_path(BATCH35_ALPHA)).convert("RGBA")
    ...
    draw.text((36, 28), "P0 Batch 51 - Suzune strict AI generation candidate", fill=(42, 36, 32), font=title_font)
    draw.text(
        (36, 76),
    ...
    draw_panel(sheet, draw, batch35, (1270, 126), (250, 250), "Batch 35 cutout baseline", label_font)
    draw_panel(sheet, draw, chroma, (1556, 126), (285, 285), "Batch 51 chroma source", label_font)
    draw_panel(sheet, draw, alpha, (1877, 126), (285, 285), "Batch 51 alpha candidate", label_font)
    ...
    notes = [
        "PASS: non-human calico cat body, blue eyes, triangular ears, warm white robe, vermilion skirt, and calico tail are present.",
        "PASS: bell wand, bell ornaments, red cords, moon-blue talismans, and tear-drop charms remain readable.",
        "WATCH: Batch 51 is more close-up and hero-polished than Batch 35; game-scale readability must be checked.",
        "WATCH: wand strings, bells, talismans, and droplets create more alpha edge complexity than the older cutout.",
    ...
    draw.text((36, 1408), "Batch 51 does not supersede Batch 35 until active-cat Unity review proves it reads better without source drift.", fill=(78, 68, 60), font=body_font)
    sheet.save(REVIEW_SHEET_PATH)


def write_review_note(rows: list[dict[str, str]]) -> None:
    lines = [
        "# Suzune Batch 51 Strict AI Generation Candidate Review",
        "",
        "Decision: candidate review only; do not import into Unity.",
        "",
        "This candidate was generated after the Batch 47 strict generation spec so it can be compared against the existing Batch 35 cutout baseline.",
        "",
        "## Output Summary",
        "",
        ...
        "## Identity Review",
        "",
        "- Present: non-human cat body, blue eyes, triangular ears, calico orange-black-white markings, warm white and vermilion shrine outfit, red cords, gold bells, moon-blue talismans, tear-drop charms, bell wand, and calico tail.",
        "- Present: warm white, vermilion, moon-blue, gold bell, and calico palette remains close to the Batch 47 palette guard.",
        "- Watch item: Batch 51 is more close-up and hero-polished than the Batch 35 cutout baseline.",
        "- Watch item: wand strings, bells, talismans, and droplets create more alpha edge complexity than the older cutout.",
        ...
        "- Batch 51 does not supersede Batch 35 until active-cat review proves the more polished close-up read is better without source drift.",
        "",
        "## Manifest Rows",
        "",
    ]
...
def write_process_note(rows: list[dict[str, str]]) -> None:
    lines = [
        "# Suzune Batch 51 Strict AI Generation Process Note",
        "",
        ...
        "",
        "Prompt goal: keep non-human calico cat body, blue eyes, warm white and vermilion shrine healer outfit, moon-blue talismans, gold bell ornaments, healer wand or branch, and Batch 47 palette while avoiding human shrine maiden drift.",
        "",
        "Chroma-key helper result: key color `#07f40d`, transparent pixels `775400/1048576`, partially transparent pixels `7200/1048576`.",
        "",
        "Rows: " + str(len(rows)),
        "",
