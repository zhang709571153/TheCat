...
$manifestRelative = "design/development/asset_candidates/starter_cats/batch_33_suzune_strict_turnaround_derivatives_2026-06-14/suzune_batch33_strict_turnaround_manifest.csv"
...
if (-not (Test-Path $manifestPath)) {
    throw "Suzune strict-turnaround manifest is missing: $manifestRelative"
}

$rows = @(Import-Csv -LiteralPath $manifestPath)
if ($rows.Count -ne $expectedTypes.Count) {
    Add-Failure "Expected $($expectedTypes.Count) Suzune candidate rows but found $($rows.Count)"
}
...
foreach ($row in $rows) {
    ...
    if ($row.candidate_path -notlike "design/development/asset_candidates/starter_cats/suzune/batch_33_suzune_strict_turnaround_derivatives_2026-06-14/*") {
        Add-Failure "$($row.candidate_path) must stay in the Suzune Batch 33 candidate directory"
    }
...
if ($reviewNotesSeen.Count -ne 1) {
    Add-Failure "Expected one Suzune review note but found $($reviewNotesSeen.Count)"
}

if ($reviewSheetsSeen.Count -ne 1) {
    Add-Failure "Expected one Suzune review sheet but found $($reviewSheetsSeen.Count)"
}
...
if ($failures.Count -gt 0) {
    ...
    throw "Suzune strict-turnaround derivative validation failed with $($failures.Count) issue(s)."
}

Write-Host "Suzune strict-turnaround derivative validation passed."
