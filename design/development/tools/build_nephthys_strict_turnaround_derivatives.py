...
BATCH_SLUG = "batch_32_nephthys_strict_turnaround_derivatives_2026-06-14"
...
MANIFEST_PATH = BATCH_DIR / "nephthys_batch32_strict_turnaround_manifest.csv"
...
CANDIDATES: tuple[CandidateAsset, ...] = (
    CandidateAsset(
        ...
        "thecat_cat_nephthys_front_view_source_reference_512_candidate_v001.png",
        (512, 512),
        (28, 38, 585, 825),
        "Front view reference crop: hood, gold script trim, crescent ornament, blue gem, pyramid prop, and raised paw.",
    ),
    CandidateAsset(
        "side_view_reference_512",
        "thecat_cat_nephthys_side_view_source_reference_512_candidate_v001.png",
        (512, 512),
        (625, 34, 1245, 825),
        "Side view reference crop: hood volume, visible ears, floating pyramid/obelisk, cloak sweep, and striped tail.",
    ),
    CandidateAsset(
        "back_view_reference_512",
        "thecat_cat_nephthys_back_view_source_reference_512_candidate_v001.png",
        (512, 512),
        (1260, 42, 1825, 825),
        "Back view reference crop: dark blue hood, vertical gold script strip, winged blue gem, ankh, cloak split, and tail.",
    ),
    CandidateAsset(
        "combat_sprite_reference_512",
        "thecat_cat_nephthys_combat_sprite_source_reference_512_candidate_v001.png",
        (512, 512),
        (28, 38, 585, 825),
    ...
    CandidateAsset(
        ...
        "thecat_cat_nephthys_hud_avatar_source_reference_256_candidate_v001.png",
        (256, 256),
        (150, 62, 545, 375),
        "HUD avatar reference crop: gold-brown tabby face, golden eyes, hood, crescent, blue tear gem, and script trim.",
    ),
    CandidateAsset(
        "skill_icon_motif_reference_128",
        "thecat_cat_nephthys_pyramid_moon_motif_source_reference_128_candidate_v001.png",
        (128, 128),
        (20, 230, 178, 418),
        "Skill icon motif reference crop: floating pyramid over inverted obelisk with cyan magic particles.",
    ),
    CandidateAsset(
        "palette_swatch_reference_256",
        "thecat_cat_nephthys_palette_swatch_source_reference_256_candidate_v001.png",
        (256, 256),
        None,
...
def main() -> None:
    ...
    review_note = CANDIDATE_DIR / "nephthys_batch32_strict_turnaround_candidate_review.md"
    review_sheet = CANDIDATE_DIR / "thecat_cat_nephthys_batch32_strict_turnaround_review_sheet.png"
    ...
    print(f"Wrote {len(rows)} Nephthys strict-turnaround candidate assets.")
...
def build_palette_swatch(source: Image.Image, size: tuple[int, int]) -> Image.Image:
    ...
    draw.text((14, 10), "Nephthys locked palette", fill=(42, 36, 32), font=title_font)
    ...
    draw.rectangle((0, 248, size[0], size[1]), fill=(16, 41, 67, 255))
    return image
...
def write_review_sheet(source: Image.Image, produced: dict[str, Path], output_path: Path) -> None:
    ...
    draw.text((36, 26), "P0 Batch 32 - Nephthys strict turnaround derivatives", fill=(42, 36, 32), font=title_font)
    ...
    rule_y = 625
    rules = (
        "Review locks: hood, gold script trim, pyramid/obelisk prop, crescent, blue gems, ankh, striped tail.",
        "Reject: Cleopatra cliche, human robe posture, generic Egyptian drift, or missing side/back anchors.",
        "Next gate: compare active screenshot 05-active-cat-nephthys.png against this source before Unity import.",
    )
    for rule in rules:
...
def write_review_note(source_path: Path, review_sheet: Path, produced: dict[str, Path], output_path: Path) -> None:
    lines = [
        "# Nephthys Batch 32 Strict Turnaround Candidate Review",
        "",
        ...
        "## Output Policy",
        "",
        "- Candidate files are under `design/development/asset_candidates/starter_cats/nephthys`.",
    ...
    lines.extend(
        [
            ...
            "## Turnaround Checks",
            "",
            "- Front: preserve gold-brown tabby face, large golden eyes, dark blue hood, crescent hood ornament, blue tear gem, gold script border, and floating pyramid/obelisk prop.",
            "- Side: preserve hood volume, visible ears, compact cat body, cloak layers, gold script trim, raised paw, floating pyramid/obelisk prop, and striped tail.",
            "- Back: preserve dark blue hood and cloak, centered vertical gold script strip, winged blue gem, ankh emblem, split cloak, and gold-brown striped tail.",
            "- Palette: preserve gold-brown tabby fur, deep navy hood/cloak, sand-gold trim, blue gems, and cyan magic particles.",
            "",
            "## Rejection Rules",
            "",
            "- Prohibited drift: generic Egyptian fantasy or generated-lineup drift over the colored turnaround.",
            "- Reject Cleopatra costume cliche, human body language, human robe posture, or human proportions.",
            "- Reject palette drift away from gold-brown fur, deep navy cloth, sand-gold trim, and blue gems.",
            "- Reject missing front, side, or back anchors including hood, script trim, pyramid prop, ankh, crescent ornament, blue gem, and striped tail.",
            "",
            "## Next Gate",
            "",
            "Capture `05-active-cat-nephthys.png` in Play Mode and compare it against the colored three-view turnaround and this Batch 32 review sheet before any Unity import approval.",
            "",
        ]
    )
...
def write_batch_summary(rows: list[dict[str, str]], source_path: Path, review_sheet: Path, review_note: Path) -> None:
    summary = BATCH_DIR / "nephthys_batch32_strict_turnaround_summary.md"
    lines = [
        "# Nephthys Batch 32 Strict Turnaround Derivatives",
        "",
        "This batch creates source-derived Nephthys candidate assets in Codex, outside Unity, using the locked colored three-view turnaround as the only visual authority.",
        "",
