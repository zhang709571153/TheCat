...
BATCH_SLUG = "batch_31_cutout_candidate_2026-06-14"
...
MANIFEST_PATH = BATCH_DIR / "saiban_batch31_cutout_manifest.csv"
REVIEW_NOTE_PATH = CANDIDATE_DIR / "saiban_batch31_cutout_candidate_review.md"
REVIEW_SHEET_PATH = CANDIDATE_DIR / "thecat_cat_saiban_batch31_cutout_review_sheet.png"
PROCESS_NOTE_PATH = CANDIDATE_DIR / "saiban_batch31_cutout_process_note.md"
...
BATCH30_CANDIDATE = REPO_ROOT / "design" / "development" / "asset_candidates" / "starter_cats" / "saiban" / "batch_30_ai_refinement_candidate_2026-06-14" / "thecat_cat_saiban_ai_refinement_combat_1024_candidate_v001.png"
BATCH30_REVIEW = REPO_ROOT / "design" / "development" / "asset_candidates" / "starter_cats" / "saiban" / "batch_30_ai_refinement_candidate_2026-06-14" / "saiban_batch30_ai_refinement_candidate_review.md"
AGENT_PROMPT = REPO_ROOT / "design" / "development" / "agent_prompts" / "p0_asset_batch_31_saiban_cutout_candidate.md"
SOURCE_LOCK_ID = "saiban_turnaround_colored"
ACTIVE_SCREENSHOT = "04-active-cat-saiban.png"

CUTOUT_NAME = "thecat_cat_saiban_cutout_alpha_1024_candidate_v001.png"
PREVIEW_NAME = "thecat_cat_saiban_cutout_alpha_512_preview_v001.png"
CHECKER_NAME = "thecat_cat_saiban_cutout_checkerboard_512_review_v001.png"
MASK_NAME = "thecat_cat_saiban_cutout_alpha_mask_512_review_v001.png"


def main() -> None:
    source_path = resolve_one(SOURCE_PATTERN)
    if not BATCH30_CANDIDATE.exists():
        raise FileNotFoundError(BATCH30_CANDIDATE)

    CANDIDATE_DIR.mkdir(parents=True, exist_ok=True)
    BATCH_DIR.mkdir(parents=True, exist_ok=True)

    source_image = Image.open(BATCH30_CANDIDATE).convert("RGBA")
    ...
    print("Wrote Saiban cutout candidate pack.")
    ...
    for y in range(0, image.height, cell):
        for x in range(0, image.width, cell):
            if ((x // cell) + (y // cell)) % 2 == 0:
                draw.rectangle((x, y, x + cell - 1, y + cell - 1), fill=(252, 252, 252, 255))
            else:
                draw.rectangle((x, y, x + cell - 1, y + cell - 1), fill=(180, 187, 196, 255))
...
def write_review_sheet(
    ...
    batch30 = Image.open(BATCH30_CANDIDATE).convert("RGBA")
    ...
    draw.text((36, 28), "P0 Batch 31 - Saiban transparent cutout candidate", fill=(42, 36, 32), font=title_font)
    ...
    batch30_panel = fit_to_canvas(batch30, (300, 300), (250, 246, 237, 255))
    ...
    draw_panel(sheet, draw, batch30_panel, (560, 116), "Batch 30 source candidate", label_font)
    ...
    checks = (
        "Candidate traits to keep: non-human cat, shield, sword, tabby face, red cape, silver armor, gold trim, blue gems.",
        "Cutout method: border flood-fill against parchment background, soft alpha edge; no source identity repaint.",
        "Watch item: fur and cape edges need Unity active-cat screenshot QA before any formal install.",
...
def write_review_note(
    ...
    lines = [
        ...
        f"- Batch 30 candidate review: `{to_repo_path(BATCH30_REVIEW)}`",
...
def write_process_note(source_path: Path, stats: dict[str, str]) -> None:
    lines = [
        ...
        f"- Input candidate: `{to_repo_path(BATCH30_CANDIDATE)}`",
...
def build_row(asset_type: str, candidate_path: Path, source_path: Path, candidate_size: str) -> dict[str, str]:
    return {
        ...
        "input_candidate_path": to_repo_path(BATCH30_CANDIDATE),
        "input_candidate_sha256": sha256(BATCH30_CANDIDATE),
...
def write_summary(
    ...
    summary = BATCH_DIR / "saiban_batch31_cutout_summary.md"
    lines = [
        "# Saiban Batch 31 Cutout Candidate",
        "",
        "This batch converts the Batch 30 Saiban candidate into an alpha PNG candidate for Unity sprite review. It remains outside Unity and is not approved for import.",
        "",
