...
$manifestRelative = "design/development/asset_candidates/starter_cats/batch_31_cutout_candidate_2026-06-14/saiban_batch31_cutout_manifest.csv"
...
if (-not (Test-Path $manifestPath)) {
    throw "Saiban cutout manifest is missing: $manifestRelative"
}

$rows = @(Import-Csv -LiteralPath $manifestPath)
if ($rows.Count -ne $expectedTypes.Count) {
    Add-Failure "Expected $($expectedTypes.Count) Saiban cutout rows but found $($rows.Count)"
}
...
foreach ($row in $rows) {
    ...
    if ($row.source_lock_id -ne "saiban_turnaround_colored") {
        Add-Failure "$($row.asset_type) source lock should be saiban_turnaround_colored but found $($row.source_lock_id)"
    }
    ...
    if ($row.candidate_path -notlike "design/development/asset_candidates/starter_cats/saiban/batch_31_cutout_candidate_2026-06-14/*") {
        Add-Failure "$($row.candidate_path) must stay in the Saiban Batch 31 candidate directory"
    }
    ...
    if ($row.source_turnaround_path -notlike "design/*/assets/characters/ch01_saiban_swordsaint/turnaround/saiban_turnaround_colored_2026-06-03.png") {
        Add-Failure "$($row.asset_type) must pin the Saiban colored turnaround source"
    }
    ...
    if (-not (Test-Path $inputCandidatePath)) {
        Add-Failure "Input Batch 30 candidate is missing: $($row.input_candidate_path)"
    } else {
        Test-Hash $inputCandidatePath $row.input_candidate_sha256 "input Batch 30 candidate"
    }
...
if ($reviewNotesSeen.Count -ne 1) {
    Add-Failure "Expected one Saiban cutout review note but found $($reviewNotesSeen.Count)"
}

if ($reviewSheetsSeen.Count -ne 1) {
    Add-Failure "Expected one Saiban cutout review sheet but found $($reviewSheetsSeen.Count)"
}

if ($processNotesSeen.Count -ne 1) {
    Add-Failure "Expected one Saiban cutout process note but found $($processNotesSeen.Count)"
}

if ($agentPromptsSeen.Count -ne 1) {
    Add-Failure "Expected one Saiban cutout agent prompt but found $($agentPromptsSeen.Count)"
}
...
if ($failures.Count -gt 0) {
    ...
    throw "Saiban cutout validation failed with $($failures.Count) issue(s)."
}

Write-Host "Saiban cutout candidate validation passed."
