...
BATCH_SLUG = "batch_43_call_tyrant_ai_refinement_candidate_2026-06-15"
...
MANIFEST_PATH = BATCH_DIR / "call_tyrant_batch43_ai_refinement_manifest.csv"
PROMPT_PATH = CANDIDATE_DIR / "call_tyrant_batch43_ai_refinement_prompt.md"
REVIEW_NOTE_PATH = CANDIDATE_DIR / "call_tyrant_batch43_ai_refinement_candidate_review.md"
REVIEW_SHEET_PATH = CANDIDATE_DIR / "thecat_enemy_call_tyrant_batch43_ai_refinement_review_sheet.png"
...
RAW_NAME = "thecat_enemy_call_tyrant_ai_refinement_raw_codex_v001.png"
CANDIDATE_NAME = "thecat_enemy_call_tyrant_ai_refinement_combat_1024_candidate_v001.png"
PREVIEW_NAME = "thecat_enemy_call_tyrant_ai_refinement_combat_512_preview_v001.png"

PROMPT_TEXT = """# Call Tyrant Batch 43 AI Refinement Prompt
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("Usage: build_call_tyrant_ai_refinement_candidate.py <generated_image_path>")
    ...
    print("Wrote Call Tyrant AI refinement candidate pack.")
...
def write_review_sheet(concept_path: Path, candidate_path: Path, preview_path: Path) -> None:
    ...
    draw.text((36, 28), "P0 Batch 43 - Call Tyrant AI refinement candidate", fill=(42, 36, 32), font=title_font)
    ...
    draw.text((36, 522), "Source authority: Call Tyrant concept", fill=(42, 36, 32), font=label_font)
    ...
    checks = (
        "Pass candidate traits to inspect: giant phone shell, red call-eye signal, purple tie, black mud body, app projectile language.",
        "Watch item: candidate remains a single-view AI refinement; summon and throw timing still comes from call_tyrant_animation.",
        "Import status: blocked until Play Mode active screenshot 09-active-enemy-call-tyrant.png is reviewed.",
    )
    y = 590
    for check in checks:
...
def draw_wrapped_text(draw: ImageDraw.ImageDraw, text: str, pos: tuple[int, int], font: ImageFont.ImageFont, width: int, fill: tuple[int, int, int]) -> int:
    ...
    for line in textwrap.wrap(text, width=width):
        draw.text((x, y), fill=fill, font=font, text=line)
        y += line_height
    return y
...
def write_review_note(concept_path: Path, animation_path: Path, generated_path: Path, raw_path: Path, candidate_path: Path, preview_path: Path) -> None:
    lines = [
        "# Call Tyrant Batch 43 AI Refinement Candidate Review",
        "",
        ...
        "## Visual Review",
        "",
        "- Pass: giant phone shell, cracked glass, red call-eye signal, purple tie, black mud body and base, app projectile language, and Boss-scale silhouette are preserved.",
        "- Pass: the candidate stays close to the source concept and avoids a human office boss body, generic phone mascot, cute robot, or clean ordinary phone drift.",
        "- Watch: this is a single-view AI refinement only; summon timing, throw timing, and portal vibration remain governed by the source animation sheet and Batch 38 reference pack.",
        "- Watch: final runtime sprite import still needs transparent or cutout treatment and Play Mode screenshot comparison.",
        "",
        "## Rejection Rules",
        "",
        "- Reject if future iterations lose the giant phone shell, red call-eye signal, purple tie, black mud body and base, app projectile language, summon portal/minion vibration feel, Boss-scale silhouette, cracked glass screen, or phone-call nightmare identity.",
...
def write_summary(concept_path: Path, animation_path: Path, candidate_path: Path, preview_path: Path) -> None:
    summary = BATCH_DIR / "call_tyrant_batch43_ai_refinement_summary.md"
    lines = [
        "# Call Tyrant Batch 43 AI Refinement Candidate",
        "",
        "This batch records the first Codex built-in image-generation Call Tyrant Boss candidate. The candidate remains outside Unity and is not approved for import.",
        "",
        f"- Source concept: `{to_repo_path(concept_path)}`",
        f"- Source animation: `{to_repo_path(animation_path)}`",
