...
$manifestRelative = "design/development/asset_candidates/starter_cats/batch_37_nephthys_cutout_candidate_2026-06-15/nephthys_batch37_cutout_manifest.csv"
...
$requiredReviewTokens = @(
    ...
    "Batch 32 review sheet",
    "Batch 36 candidate review",
    "gold-brown tabby",
    "golden eyes",
    "deep navy hood",
    "crescent ornament",
...
if (-not (Test-Path $manifestPath)) {
    throw "Nephthys cutout manifest is missing: $manifestRelative"
}

$rows = @(Import-Csv -LiteralPath $manifestPath)
if ($rows.Count -ne $expectedTypes.Count) {
    Add-Failure "Expected $($expectedTypes.Count) Nephthys cutout rows but found $($rows.Count)"
}
...
foreach ($row in $rows) {
    ...
    if ($row.candidate_path -notlike "design/development/asset_candidates/starter_cats/nephthys/batch_37_nephthys_cutout_candidate_2026-06-15/*") {
        Add-Failure "$($row.candidate_path) must stay in the Nephthys Batch 37 candidate directory"
    }
    ...
    if (-not (Test-Path $inputCandidatePath)) {
        Add-Failure "Input Batch 36 candidate is missing: $($row.input_candidate_path)"
    } else {
        Test-Hash $inputCandidatePath $row.input_candidate_sha256 "input Batch 36 candidate"
    }
...
if ($reviewNotesSeen.Count -ne 1) {
    Add-Failure "Expected one Nephthys cutout review note but found $($reviewNotesSeen.Count)"
}

if ($reviewSheetsSeen.Count -ne 1) {
    Add-Failure "Expected one Nephthys cutout review sheet but found $($reviewSheetsSeen.Count)"
}

if ($processNotesSeen.Count -ne 1) {
    Add-Failure "Expected one Nephthys cutout process note but found $($processNotesSeen.Count)"
}

if ($agentPromptsSeen.Count -ne 1) {
    Add-Failure "Expected one Nephthys cutout agent prompt but found $($agentPromptsSeen.Count)"
}
...
if ($failures.Count -gt 0) {
    ...
    throw "Nephthys cutout validation failed with $($failures.Count) issue(s)."
}

Write-Host "Nephthys cutout candidate validation passed."
