...
$batchSlug = "batch_41_cold_light_ai_refinement_candidate_2026-06-15"
$candidateRoot = "design/development/asset_candidates/enemies/cold_light_shadow/$batchSlug"
$manifestRelative = "design/development/asset_candidates/enemies/$batchSlug/cold_light_batch41_ai_refinement_manifest.csv"
...
$requiredPromptTokens = @(
    "corrupted mechanical desk-lamp nightmare enemy",
    "cold cyan lamp head glow",
    "one hostile red eye",
    "black mud dripping",
    "black mud base",
    "ranged beam pressure",
    "Avoid"
)
...
if (-not (Test-Path -LiteralPath $manifestPath)) {
    throw "Cold Light Batch 41 AI refinement manifest is missing: $manifestRelative"
}

$rows = @(Import-Csv -LiteralPath $manifestPath)
if ($rows.Count -ne $expectedTypes.Count) {
    Add-Failure "Expected $($expectedTypes.Count) Cold Light AI refinement rows but found $($rows.Count)"
}
...
foreach ($row in $rows) {
    ...
    if ($row.candidate_path -notlike "design/development/asset_candidates/enemies/cold_light_shadow/$batchSlug/*") {
        Add-Failure "$($row.candidate_path) must stay in the Cold Light Batch 41 candidate directory"
    }
...
if ($reviewNotesSeen.Count -ne 1) {
    Add-Failure "Expected one Cold Light AI review note but found $($reviewNotesSeen.Count)"
}

if ($reviewSheetsSeen.Count -ne 1) {
    Add-Failure "Expected one Cold Light AI review sheet but found $($reviewSheetsSeen.Count)"
}

if ($promptRecordsSeen.Count -ne 1) {
    Add-Failure "Expected one Cold Light AI prompt record but found $($promptRecordsSeen.Count)"
}
...
if ($metaFiles.Count -gt 0) {
    Add-Failure "Batch 41 candidate folder must not contain Unity .meta files."
}
...
if ($failures.Count -gt 0) {
    ...
    throw "Cold Light AI refinement validation failed with $($failures.Count) issue(s)."
}

Write-Host "Cold Light AI refinement candidate validation passed."
