...
$batchSlug = "batch_49_saiban_low_drift_refinement_2026-06-15"
...
$manifestRelative = "$batchRootRelative/saiban_batch49_low_drift_refinement_manifest.csv"
$manifestPath = Join-Path $projectRoot $manifestRelative
$reviewSheetRelative = "$catDirRelative/thecat_cat_saiban_batch49_low_drift_refinement_review_sheet.png"
$reviewNoteRelative = "$catDirRelative/saiban_batch49_low_drift_refinement_review.md"
$processNoteRelative = "$catDirRelative/saiban_batch49_low_drift_refinement_process_note.md"
$agentPromptRelative = "design/development/agent_prompts/p0_asset_batch_49_saiban_low_drift_refinement.md"
...
$requiredReviewTokens = @(
    ...
    "armor ornamentation is reduced",
    "side/back identity anchors",
    "No Unity"
)
...
if (-not (Test-Path -LiteralPath $manifestPath)) {
    throw "Batch 49 Saiban low-drift refinement manifest is missing: $manifestRelative"
}

$rows = @(Import-Csv -LiteralPath $manifestPath -Encoding UTF8)
if ($rows.Count -ne 7) {
    Add-Failure "Expected 7 Saiban Batch 49 rows but found $($rows.Count)"
}
...
foreach ($row in $rows) {
    ...
    if ($row.active_screenshot -ne "05-active-cat-saiban.png") {
        Add-Failure "$($row.asset_type) active screenshot should be 05-active-cat-saiban.png but found $($row.active_screenshot)"
    }
    ...
    if ($row.candidate_path -notlike "$catDirRelative/*") {
        Add-Failure "$($row.asset_type) candidate path must stay in the Batch 49 Saiban directory"
    }
    ...
    if ($row.batch48_alpha_path -notlike "design/development/asset_candidates/starter_cats/saiban/batch_48_saiban_ai_generation_pilot_2026-06-15/*.png") {
        Add-Failure "$($row.asset_type) must reference the Saiban Batch 48 alpha candidate"
    }
    ...
    Test-Hash (Resolve-ProjectPath $row.batch48_alpha_path) $row.batch48_alpha_sha256 "$($row.asset_type) Batch 48 alpha"
    Test-Hash (Resolve-ProjectPath $row.batch48_review_path) $row.batch48_review_sha256 "$($row.asset_type) Batch 48 review"
...
if (-not (Test-Path -LiteralPath $reviewSheetPath)) {
    Add-Failure "Batch 49 review sheet is missing: $reviewSheetRelative"
} else {
    $sheetSize = Get-ImageSize $reviewSheetPath
    if ($sheetSize -ne "2200x1450") {
        Add-Failure "Batch 49 review sheet should be 2200x1450 but is $sheetSize"
    }

    if (Test-Path -LiteralPath "$reviewSheetPath.meta") {
        Add-Failure "Batch 49 review sheet must not have a Unity .meta file"
    }
}
...
foreach ($textRelative in @($reviewNoteRelative, $processNoteRelative, $agentPromptRelative)) {
    ...
    if (-not (Test-Path -LiteralPath $textPath)) {
        Add-Failure "Required Batch 49 text file is missing: $textRelative"
    }
}
...
if ($metaFiles.Count -gt 0) {
    Add-Failure "Batch 49 must not create Unity .meta files: $($metaFiles.FullName -join ', ')"
}
...
if ($failures.Count -gt 0) {
    ...
    throw "Saiban Batch 49 low-drift refinement validation failed with $($failures.Count) issue(s)."
}

Write-Host "Saiban Batch 49 low-drift refinement validation passed."
