...
$manifestRelative = "design/development/asset_candidates/starter_cats/batch_30_ai_refinement_candidate_2026-06-14/saiban_batch30_ai_refinement_manifest.csv"
...
if (-not (Test-Path $manifestPath)) {
    throw "Saiban AI refinement manifest is missing: $manifestRelative"
}

$rows = @(Import-Csv -LiteralPath $manifestPath)
if ($rows.Count -ne $expectedTypes.Count) {
    Add-Failure "Expected $($expectedTypes.Count) Saiban AI refinement rows but found $($rows.Count)"
}
...
foreach ($row in $rows) {
    ...
    if ($row.candidate_path -notlike "design/development/asset_candidates/starter_cats/saiban/batch_30_ai_refinement_candidate_2026-06-14/*") {
        Add-Failure "$($row.candidate_path) must stay in the Saiban Batch 30 candidate directory"
    }
...
if ($reviewNotesSeen.Count -ne 1) {
    Add-Failure "Expected one Saiban AI review note but found $($reviewNotesSeen.Count)"
}

if ($reviewSheetsSeen.Count -ne 1) {
    Add-Failure "Expected one Saiban AI review sheet but found $($reviewSheetsSeen.Count)"
}

if ($promptRecordsSeen.Count -ne 1) {
    Add-Failure "Expected one Saiban AI prompt record but found $($promptRecordsSeen.Count)"
}
...
foreach ($promptRelative in $promptRecordsSeen) {
    ...
    foreach ($token in @("shield", "sword", "tabby", "red cape", "silver armor", "blue gems", "Avoid")) {
        if ($content.IndexOf($token, [System.StringComparison]::OrdinalIgnoreCase) -lt 0) {
...
if ($failures.Count -gt 0) {
    ...
    throw "Saiban AI refinement validation failed with $($failures.Count) issue(s)."
}

Write-Host "Saiban AI refinement candidate validation passed."
