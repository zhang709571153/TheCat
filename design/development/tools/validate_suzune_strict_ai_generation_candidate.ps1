...
$batchSlug = "batch_51_suzune_strict_ai_generation_candidate_2026-06-15"
$catDirRelative = "design/development/asset_candidates/starter_cats/suzune/$batchSlug"
$batchRootRelative = "design/development/asset_candidates/starter_cats/$batchSlug"
$manifestRelative = "$batchRootRelative/suzune_batch51_strict_ai_generation_manifest.csv"
$manifestPath = Join-Path $projectRoot $manifestRelative
$reviewSheetRelative = "$catDirRelative/thecat_cat_suzune_batch51_strict_ai_generation_review_sheet.png"
$reviewNoteRelative = "$catDirRelative/suzune_batch51_strict_ai_generation_candidate_review.md"
$processNoteRelative = "$catDirRelative/suzune_batch51_strict_ai_generation_process_note.md"
$agentPromptRelative = "design/development/agent_prompts/p0_asset_batch_51_suzune_strict_ai_generation_candidate.md"
...
if (-not (Test-Path -LiteralPath $manifestPath)) {
    throw "Batch 51 Suzune strict AI generation manifest is missing: $manifestRelative"
}

$rows = @(Import-Csv -LiteralPath $manifestPath -Encoding UTF8)
if ($rows.Count -ne 7) {
    Add-Failure "Expected 7 Suzune Batch 51 rows but found $($rows.Count)"
}
...
foreach ($row in $rows) {
    ...
    if ($row.active_screenshot -ne "07-active-cat-suzune.png") {
        Add-Failure "$($row.asset_type) active screenshot should be 07-active-cat-suzune.png but found $($row.active_screenshot)"
    }
    ...
    if ($row.candidate_path -notlike "$catDirRelative/*") {
        Add-Failure "$($row.asset_type) candidate path must stay in the Batch 51 Suzune directory"
    }
    ...
    if ($row.batch35_alpha_path -notlike "design/development/asset_candidates/starter_cats/suzune/batch_35_suzune_cutout_candidate_2026-06-15/*.png") {
        Add-Failure "$($row.asset_type) must reference the Suzune Batch 35 alpha candidate"
    }
    ...
    Test-Hash (Resolve-ProjectPath $row.batch35_alpha_path) $row.batch35_alpha_sha256 "$($row.asset_type) Batch 35 alpha"
    Test-Hash (Resolve-ProjectPath $row.batch35_review_path) $row.batch35_review_sha256 "$($row.asset_type) Batch 35 review"
...
if (-not (Test-Path -LiteralPath $reviewSheetPath)) {
    Add-Failure "Batch 51 review sheet is missing: $reviewSheetRelative"
} else {
    $sheetSize = Get-ImageSize $reviewSheetPath
    if ($sheetSize -ne "2200x1450") {
        Add-Failure "Batch 51 review sheet should be 2200x1450 but is $sheetSize"
    }

    if (Test-Path -LiteralPath "$reviewSheetPath.meta") {
        Add-Failure "Batch 51 review sheet must not have a Unity .meta file"
    }
}
...
foreach ($textRelative in @($reviewNoteRelative, $processNoteRelative, $agentPromptRelative)) {
    ...
    if (-not (Test-Path -LiteralPath $textPath)) {
        Add-Failure "Required Batch 51 text file is missing: $textRelative"
    }
}
...
if ($metaFiles.Count -gt 0) {
    Add-Failure "Batch 51 must not create Unity .meta files: $($metaFiles.FullName -join ', ')"
}
...
if ($failures.Count -gt 0) {
    ...
    throw "Suzune Batch 51 strict AI generation validation failed with $($failures.Count) issue(s)."
}

Write-Host "Suzune Batch 51 strict AI generation validation passed."
