...
$batchSlug = "batch_106_main_menu_entry_tokens_2026-06-26"
$batchDirRelative = "design/development/asset_candidates/ui/main_menu_entry_tokens/$batchSlug"
$reviewNoteRelative = "design/development/asset_review/BATCH106_MAIN_MENU_ENTRY_TOKENS_AGENT_REVIEW_2026-06-26.md"
...
function Assert-Exists {
    ...
    if (-not (Test-Path -LiteralPath (Resolve-ProjectPath $Relative))) {
        Add-Failure "Missing required Batch 106 file: $Relative"
    }
}
...
$requiredFiles = @(
    "$batchDirRelative/source/tc_ui_menu_batch106_chromakey_source_v001.png",
    "$batchDirRelative/source/tc_ui_menu_batch106_names.txt",
    "$batchDirRelative/alpha/tc_ui_menu_batch106_alpha_sheet_v001.png",
    "$batchDirRelative/tc_ui_menu_batch_106_main_menu_entry_tokens_2026-06-26_asset_table.csv",
    "$batchDirRelative/tc_ui_menu_batch106_semantic_manifest.csv",
    "$batchDirRelative/tc_ui_menu_batch106_semantic_contact_sheet_v001.png",
    "$batchDirRelative/tc_ui_menu_batch106_64px_main_menu_readability_board_v001.png",
    "$batchDirRelative/tc_ui_menu_batch_106_main_menu_entry_tokens_2026-06-26_final_review.csv",
    "$batchDirRelative/tc_ui_menu_batch_106_main_menu_entry_tokens_2026-06-26_process_note.md",
    "$batchDirRelative/BATCH106_MAIN_MENU_ENTRY_TOKENS_AGENT_REVIEW_2026-06-26.md",
    "$batchDirRelative/reviews/BATCH106_MAIN_MENU_ENTRY_TOKENS_AGENT_REVIEW_2026-06-26.md",
    "$batchDirRelative/reviews/tc_ui_menu_batch_106_main_menu_entry_tokens_2026-06-26_candidate_review.md",
    $reviewNoteRelative
)
...
if (-not (Test-Path -LiteralPath $batchDir)) {
    throw "Batch 106 directory is missing: $batchDirRelative"
}

$expectedNames = @(
    "start_dream_gate_token",
    "continue_run_moon_token",
    "cat_room_home_token",
    "dream_route_map_token",
    "collection_book_token",
    "settings_cog_token",
    "exit_door_token",
    "notice_bell_token",
    "disabled_lock_token"
...
$namesPath = Resolve-ProjectPath "$batchDirRelative/source/tc_ui_menu_batch106_names.txt"
if (Test-Path -LiteralPath $namesPath) {
    $actualNames = @(Get-Content -LiteralPath $namesPath | Where-Object { $_.Trim().Length -gt 0 })
    if (($actualNames -join "|") -ne ($expectedNames -join "|")) {
        Add-Failure "Names file does not match expected Batch 106 semantic order"
    }
}

$assetRows = @(Import-Csv -LiteralPath (Resolve-ProjectPath "$batchDirRelative/tc_ui_menu_batch_106_main_menu_entry_tokens_2026-06-26_asset_table.csv"))
if ($assetRows.Count -ne 9) {
    Add-Failure "Asset table should contain 9 rows, found $($assetRows.Count)"
}

$manifestRows = @(Import-Csv -LiteralPath (Resolve-ProjectPath "$batchDirRelative/tc_ui_menu_batch106_semantic_manifest.csv"))
...
foreach ($row in $assetRows) {
    ...
    if ($row.target_use -ne "candidate_only_main_menu_ui_tokens") {
...
$reviewRows = @(Import-Csv -LiteralPath (Resolve-ProjectPath "$batchDirRelative/tc_ui_menu_batch_106_main_menu_entry_tokens_2026-06-26_final_review.csv"))
...
foreach ($row in $reviewRows) {
    ...
    if ($row.sorting_layer -ne "UI canvas menu token slot") {
...
Assert-Contains "$batchDirRelative/tc_ui_menu_batch_106_main_menu_entry_tokens_2026-06-26_process_note.md" @(
    "built-in Codex",
    "imagegen",
...
$rootReview = Resolve-ProjectPath "$batchDirRelative/BATCH106_MAIN_MENU_ENTRY_TOKENS_AGENT_REVIEW_2026-06-26.md"
$localReview = Resolve-ProjectPath "$batchDirRelative/reviews/BATCH106_MAIN_MENU_ENTRY_TOKENS_AGENT_REVIEW_2026-06-26.md"
$candidateReview = Resolve-ProjectPath "$batchDirRelative/reviews/tc_ui_menu_batch_106_main_menu_entry_tokens_2026-06-26_candidate_review.md"
...
if ((Test-Path -LiteralPath $rootReview) -and (Test-Path -LiteralPath $localReview) -and (Test-Path -LiteralPath $candidateReview) -and (Test-Path -LiteralPath $mirroredReview)) {
    ...
    if ($rootHash -ne $localHash -or $rootHash -ne $candidateHash -or $rootHash -ne $mirrorHash) {
        Add-Failure "Batch 106 review-note mirrors differ"
    }
}

$metaFiles = @(Get-ChildItem -LiteralPath $batchDir -Recurse -Filter "*.meta" -File)
if ($metaFiles.Count -gt 0) {
    Add-Failure "Batch 106 candidate directory must not contain Unity .meta files"
}

$assetLeak = @(Get-ChildItem -LiteralPath (Resolve-ProjectPath "Assets") -Recurse -File -ErrorAction SilentlyContinue | Where-Object {
    $_.Name -like "*batch106*" -or
    $_.Name -like "*main_menu_entry_tokens*" -or
    $_.Name -like "*tc_ui_menu*batch106*"
})
if ($assetLeak.Count -gt 0) {
    Add-Failure "Batch 106 must not write runtime Assets files before approval"
}

if ($failures.Count -gt 0) {
    Write-Error ("Batch 106 main-menu entry token validation failed:`n" + ($failures -join "`n"))
    exit 1
}

Write-Output "Batch 106 main-menu entry token validation passed. Rows: $($manifestRows.Count); Manifest: $batchDirRelative/tc_ui_menu_batch106_semantic_manifest.csv"
