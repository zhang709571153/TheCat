...
$batchSlug = "batch_43_call_tyrant_ai_refinement_candidate_2026-06-15"
$candidateRoot = "design/development/asset_candidates/enemies/call_tyrant/$batchSlug"
$manifestRelative = "design/development/asset_candidates/enemies/$batchSlug/call_tyrant_batch43_ai_refinement_manifest.csv"
...
$requiredPromptTokens = @(
    "corrupted ringing-phone nightmare Boss",
    "giant cracked black smartphone shell",
    "red glowing phone-call icon eyes",
    "purple necktie",
    "black mud",
    "app projectile language",
    "summon and throw pressure",
    "Avoid"
)
...
if (-not (Test-Path -LiteralPath $manifestPath)) {
    throw "Call Tyrant Batch 43 AI refinement manifest is missing: $manifestRelative"
}

$rows = @(Import-Csv -LiteralPath $manifestPath)
if ($rows.Count -ne $expectedTypes.Count) {
    Add-Failure "Expected $($expectedTypes.Count) Call Tyrant AI refinement rows but found $($rows.Count)"
}
...
foreach ($row in $rows) {
    ...
    if ($row.candidate_path -notlike "design/development/asset_candidates/enemies/call_tyrant/$batchSlug/*") {
        Add-Failure "$($row.candidate_path) must stay in the Call Tyrant Batch 43 candidate directory"
    }
...
if ($reviewNotesSeen.Count -ne 1) {
    Add-Failure "Expected one Call Tyrant AI review note but found $($reviewNotesSeen.Count)"
}

if ($reviewSheetsSeen.Count -ne 1) {
    Add-Failure "Expected one Call Tyrant AI review sheet but found $($reviewSheetsSeen.Count)"
}

if ($promptRecordsSeen.Count -ne 1) {
    Add-Failure "Expected one Call Tyrant AI prompt record but found $($promptRecordsSeen.Count)"
}
...
if ($metaFiles.Count -gt 0) {
    Add-Failure "Batch 43 candidate folder must not contain Unity .meta files."
}
...
if ($failures.Count -gt 0) {
    ...
    throw "Call Tyrant AI refinement validation failed with $($failures.Count) issue(s)."
}

Write-Host "Call Tyrant AI refinement candidate validation passed."
