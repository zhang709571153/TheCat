...
| B-007 | Full offline/visual acceptance remains blocked by starter-cat asset evidence state | `P0_OFFLINE_ACCEPTANCE_REPORT.md` and the D1 consumer batch show the remaining disagreement is around starter-cat source-lock/turnaround/strict-candidate evidence and active screenshot/formal-import expectations, not entry, cat-room, route, or dream-map behavior | Keep starter-cat body replacement locked; repair evidence packets only after comparing active-cat screenshots against colored turnarounds |
...
| Starter-cat body replacement candidates | Blocked, not retired | Keep as review-only candidates until active-cat screenshots pass colored-turnaround comparison |
