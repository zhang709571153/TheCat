...
{
    public static class P0LoadingStartBatch83RuntimeEvidenceBatchmodeRunner
    {
        private const double EnterPlayModeTimeoutSeconds = 45d;
        private const double RuntimeEvidenceTimeoutSeconds = 160d;
        private const string CommandLineEntryPoint = "TheCat.EditorTools.P0LoadingStartBatch83RuntimeEvidenceBatchmodeRunner.RunForBatchmode";
        ...
        public static void RunForBatchmode()
        {
            try
            {
                ...
                hasStarted = true;
                Debug.Log("[TheCat] Batch 83 loading/start runtime evidence runner invoked.");
                ResetState();
                ...
                Debug.Log("[TheCat] Entering Play Mode for Batch 83 loading/start runtime evidence.");
        ...
        {
            Debug.Log("[TheCat] Batch 83 runtime evidence Play Mode state changed: " + change + ".");
        ...
        private static void Update()
        {
            ...
            try
            {
                ...
                if (!evidenceStarted)
                {
                    ...
                    {
                        ExitWithFailure("Timed out entering Play Mode before starting Batch 83 runtime evidence.");
                    }

                    return;
                }

                if (P0LoadingStartBatch83RuntimeEvidence.IsFinished)
                {
                    bool passed = P0LoadingStartBatch83RuntimeEvidence.State == P0LoadingStartBatch83RuntimeEvidenceState.Passed
                        && P0LoadingStartBatch83RuntimeEvidence.CapturedPaths.Count == P0LoadingStartBatch83RuntimeEvidence.ExpectedScreenshotCount
                        && P0LoadingStartBatch83RuntimeEvidence.GeneratedReviewPaths.Count == P0LoadingStartBatch83RuntimeEvidence.ExpectedAutomaticReviewCount;
                    CleanupAndExit(passed, P0LoadingStartBatch83RuntimeEvidence.LastSummary);
                    return;
                }

                if (elapsed > RuntimeEvidenceTimeoutSeconds)
                {
                    ExitWithFailure("Timed out waiting for Batch 83 loading/start runtime evidence to finish.");
                }
            }
            catch (Exception exception)
            {
        ...
        private static void StartEvidence()
        {
            ...
            Debug.Log("[TheCat] Starting Batch 83 loading/start runtime evidence.");
            P0LoadingStartBatch83RuntimeEvidence.SetBeforeCaptureResolutionOverride(P0LoadingStartBatch83GameViewSizeUtility.ApplyTargetSize);
            bool started = P0LoadingStartBatch83RuntimeEvidence.StartDefaultRuntimeEvidence();
            if (!started)
            {
                ExitWithFailure("Could not start Batch 83 runtime evidence: " + P0LoadingStartBatch83RuntimeEvidence.LastSummary);
            }
        }
        ...
        private static void CleanupAndExit(bool passed, string message)
        {
            ...
            string logMessage = "[TheCat] Batch 83 loading/start runtime evidence " + (passed ? "passed: " : "failed: ") + message;
            if (passed)
            {
        ...
        {
            string path = ToProjectPath(P0LoadingStartBatch83RuntimeEvidence.RuntimeReportPath);
            ...
            File.WriteAllText(
                ...
                P0LoadingStartBatch83RuntimeEvidence.BuildRuntimeReportMarkdown(
                    ...
                    message,
                    message,
                    P0LoadingStartBatch83RuntimeEvidence.CapturedPaths,
                    P0LoadingStartBatch83RuntimeEvidence.GeneratedReviewPaths));
        }
    ...
    internal static class P0LoadingStartBatch83GameViewSizeUtility
    {
        private const string CustomSizePrefix = "Batch83 ";

        public static string ApplyTargetSize(P0LoadingStartBatch83ScreenshotTarget target)
        {
            try
            {
