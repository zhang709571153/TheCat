...
{
    public static class P0BattleHudBatch87RuntimeEvidenceBatchmodeRunner
    {
        private const double EnterPlayModeTimeoutSeconds = 45d;
        private const double RuntimeEvidenceTimeoutSeconds = 160d;
        private const string CommandLineEntryPoint = "TheCat.EditorTools.P0BattleHudBatch87RuntimeEvidenceBatchmodeRunner.RunForBatchmode";
        ...
        public static void RunForBatchmode()
        {
            try
            {
                ...
                hasStarted = true;
                Debug.Log("[TheCat] Batch 87 battle HUD runtime evidence runner invoked.");
                ResetState();
                ...
                Debug.Log("[TheCat] Entering Play Mode for Batch 87 battle HUD runtime evidence.");
        ...
        {
            Debug.Log("[TheCat] Batch 87 runtime evidence Play Mode state changed: " + change + ".");
        ...
        private static void Update()
        {
            ...
            try
            {
                ...
                if (!evidenceStarted)
                {
                    ...
                    {
                        ExitWithFailure("Timed out entering Play Mode before starting Batch 87 runtime evidence.");
                    }

                    return;
                }

                if (P0BattleHudBatch87RuntimeEvidence.IsFinished)
                {
                    bool passed = P0BattleHudBatch87RuntimeEvidence.State == P0BattleHudBatch87RuntimeEvidenceState.Passed
                        && P0BattleHudBatch87RuntimeEvidence.CapturedPaths.Count == P0BattleHudBatch87RuntimeEvidence.ExpectedScreenshotCount
                        && P0BattleHudBatch87RuntimeEvidence.GeneratedReviewPaths.Count == P0BattleHudBatch87RuntimeEvidence.ExpectedAutomaticReviewCount;
                    CleanupAndExit(passed, P0BattleHudBatch87RuntimeEvidence.LastSummary);
                    return;
                }

                if (elapsed > RuntimeEvidenceTimeoutSeconds)
                {
                    ExitWithFailure("Timed out waiting for Batch 87 battle HUD runtime evidence to finish.");
                }
            }
            catch (Exception exception)
            {
        ...
        private static void StartEvidence()
        {
            ...
            Debug.Log("[TheCat] Starting Batch 87 battle HUD runtime evidence.");
            P0BattleHudBatch87RuntimeEvidence.SetBeforeCaptureResolutionOverride(P0BattleHudBatch87GameViewSizeUtility.ApplyTargetSize);
            bool started = P0BattleHudBatch87RuntimeEvidence.StartDefaultRuntimeEvidence();
            if (!started)
            {
                ExitWithFailure("Could not start Batch 87 runtime evidence: " + P0BattleHudBatch87RuntimeEvidence.LastSummary);
            }
        }
        ...
        private static void CleanupAndExit(bool passed, string message)
        {
            ...
            string logMessage = "[TheCat] Batch 87 battle HUD runtime evidence " + (passed ? "passed: " : "failed: ") + message;
            if (passed)
            {
        ...
        {
            string path = ToProjectPath(P0BattleHudBatch87RuntimeEvidence.RuntimeReportPath);
            ...
            File.WriteAllText(
                ...
                P0BattleHudBatch87RuntimeEvidence.BuildRuntimeReportMarkdown(
                    ...
                    message,
                    message,
                    P0BattleHudBatch87RuntimeEvidence.CapturedPaths,
                    P0BattleHudBatch87RuntimeEvidence.GeneratedReviewPaths));
        }
    ...
    internal static class P0BattleHudBatch87GameViewSizeUtility
    {
        private const string CustomSizePrefix = "Batch87 ";

        public static string ApplyTargetSize(P0BattleHudBatch87ScreenshotTarget target)
        {
            try
            {
