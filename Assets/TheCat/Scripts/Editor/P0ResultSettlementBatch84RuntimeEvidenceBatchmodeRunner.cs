...
{
    public static class P0ResultSettlementBatch84RuntimeEvidenceBatchmodeRunner
    {
        private const double EnterPlayModeTimeoutSeconds = 45d;
        private const double RuntimeEvidenceTimeoutSeconds = 160d;
        private const string CommandLineEntryPoint = "TheCat.EditorTools.P0ResultSettlementBatch84RuntimeEvidenceBatchmodeRunner.RunForBatchmode";
        ...
        public static void RunForBatchmode()
        {
            try
            {
                ...
                hasStarted = true;
                Debug.Log("[TheCat] Batch 84 result/settlement runtime evidence runner invoked.");
                ResetState();
                ...
                Debug.Log("[TheCat] Entering Play Mode for Batch 84 result/settlement runtime evidence.");
        ...
        {
            Debug.Log("[TheCat] Batch 84 runtime evidence Play Mode state changed: " + change + ".");
        ...
        private static void Update()
        {
            ...
            try
            {
                ...
                if (!evidenceStarted)
                {
                    ...
                    {
                        ExitWithFailure("Timed out entering Play Mode before starting Batch 84 runtime evidence.");
                    }

                    return;
                }

                if (P0ResultSettlementBatch84RuntimeEvidence.IsFinished)
                {
                    bool passed = P0ResultSettlementBatch84RuntimeEvidence.State == P0ResultSettlementBatch84RuntimeEvidenceState.Passed
                        && P0ResultSettlementBatch84RuntimeEvidence.CapturedPaths.Count == P0ResultSettlementBatch84RuntimeEvidence.ExpectedScreenshotCount
                        && P0ResultSettlementBatch84RuntimeEvidence.GeneratedReviewPaths.Count == P0ResultSettlementBatch84RuntimeEvidence.ExpectedAutomaticReviewCount;
                    CleanupAndExit(passed, P0ResultSettlementBatch84RuntimeEvidence.LastSummary);
                    return;
                }

                if (elapsed > RuntimeEvidenceTimeoutSeconds)
                {
                    ExitWithFailure("Timed out waiting for Batch 84 result/settlement runtime evidence to finish.");
                }
            }
            catch (Exception exception)
            {
        ...
        private static void StartEvidence()
        {
            ...
            Debug.Log("[TheCat] Starting Batch 84 result/settlement runtime evidence.");
            P0ResultSettlementBatch84RuntimeEvidence.SetBeforeCaptureResolutionOverride(P0ResultSettlementBatch84GameViewSizeUtility.ApplyTargetSize);
            bool started = P0ResultSettlementBatch84RuntimeEvidence.StartDefaultRuntimeEvidence();
            if (!started)
            {
                ExitWithFailure("Could not start Batch 84 runtime evidence: " + P0ResultSettlementBatch84RuntimeEvidence.LastSummary);
            }
        }
        ...
        private static void CleanupAndExit(bool passed, string message)
        {
            ...
            string logMessage = "[TheCat] Batch 84 result/settlement runtime evidence " + (passed ? "passed: " : "failed: ") + message;
            if (passed)
            {
        ...
        {
            string path = ToProjectPath(P0ResultSettlementBatch84RuntimeEvidence.RuntimeReportPath);
            ...
            File.WriteAllText(
                ...
                P0ResultSettlementBatch84RuntimeEvidence.BuildRuntimeReportMarkdown(
                    ...
                    message,
                    message,
                    P0ResultSettlementBatch84RuntimeEvidence.CapturedPaths,
                    P0ResultSettlementBatch84RuntimeEvidence.GeneratedReviewPaths));
        }
    ...
    internal static class P0ResultSettlementBatch84GameViewSizeUtility
    {
        private const string CustomSizePrefix = "Batch84 ";

        public static string ApplyTargetSize(P0ResultSettlementBatch84ScreenshotTarget target)
        {
            try
            {
