...
{
    public static class P0SkillSelectionBatch89RuntimeEvidenceBatchmodeRunner
    {
        private const double EnterPlayModeTimeoutSeconds = 45d;
        private const double RuntimeEvidenceTimeoutSeconds = 160d;
        private const string CommandLineEntryPoint = "TheCat.EditorTools.P0SkillSelectionBatch89RuntimeEvidenceBatchmodeRunner.RunForBatchmode";
        ...
        public static void RunForBatchmode()
        {
            try
            {
                ...
                hasStarted = true;
                Debug.Log("[TheCat] Batch 89 skill-selection runtime evidence runner invoked.");
                ResetState();
                ...
                Debug.Log("[TheCat] Entering Play Mode for Batch 89 skill-selection runtime evidence.");
        ...
        {
            Debug.Log("[TheCat] Batch 89 runtime evidence Play Mode state changed: " + change + ".");
        ...
        private static void Update()
        {
            ...
            try
            {
                ...
                if (!evidenceStarted)
                {
                    ...
                    {
                        ExitWithFailure("Timed out entering Play Mode before starting Batch 89 runtime evidence.");
                    }

                    return;
                }

                if (P0SkillSelectionBatch89RuntimeEvidence.IsFinished)
                {
                    bool passed = P0SkillSelectionBatch89RuntimeEvidence.State == P0SkillSelectionBatch89RuntimeEvidenceState.Passed
                        && P0SkillSelectionBatch89RuntimeEvidence.CapturedPaths.Count == P0SkillSelectionBatch89RuntimeEvidence.ExpectedScreenshotCount
                        && P0SkillSelectionBatch89RuntimeEvidence.GeneratedReviewPaths.Count == P0SkillSelectionBatch89RuntimeEvidence.ExpectedAutomaticReviewCount;
                    CleanupAndExit(passed, P0SkillSelectionBatch89RuntimeEvidence.LastSummary);
                    return;
                }

                if (elapsed > RuntimeEvidenceTimeoutSeconds)
                {
                    ExitWithFailure("Timed out waiting for Batch 89 skill-selection runtime evidence to finish.");
                }
            }
            catch (Exception exception)
            {
        ...
        private static void StartEvidence()
        {
            ...
            Debug.Log("[TheCat] Starting Batch 89 skill-selection runtime evidence.");
            P0SkillSelectionBatch89RuntimeEvidence.SetBeforeCaptureResolutionOverride(P0SkillSelectionBatch89GameViewSizeUtility.ApplyTargetSize);
            bool started = P0SkillSelectionBatch89RuntimeEvidence.StartDefaultRuntimeEvidence();
            if (!started)
            {
                ExitWithFailure("Could not start Batch 89 runtime evidence: " + P0SkillSelectionBatch89RuntimeEvidence.LastSummary);
            }
        }
        ...
        private static void CleanupAndExit(bool passed, string message)
        {
            ...
            string logMessage = "[TheCat] Batch 89 skill-selection runtime evidence " + (passed ? "passed: " : "failed: ") + message;
            if (passed)
            {
        ...
        {
            string path = ToProjectPath(P0SkillSelectionBatch89RuntimeEvidence.RuntimeReportPath);
            ...
            File.WriteAllText(
                ...
                P0SkillSelectionBatch89RuntimeEvidence.BuildRuntimeReportMarkdown(
                    ...
                    message,
                    message,
                    P0SkillSelectionBatch89RuntimeEvidence.CapturedPaths,
                    P0SkillSelectionBatch89RuntimeEvidence.GeneratedReviewPaths));
        }
    ...
    internal static class P0SkillSelectionBatch89GameViewSizeUtility
    {
        private const string CustomSizePrefix = "Batch89 ";

        public static string ApplyTargetSize(P0SkillSelectionBatch89ScreenshotTarget target)
        {
            try
            {
