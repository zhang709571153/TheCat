...
{
    public static class P0DreamRouteBatch86RuntimeEvidenceBatchmodeRunner
    {
        private const double EnterPlayModeTimeoutSeconds = 45d;
        private const double RuntimeEvidenceTimeoutSeconds = 160d;
        private const string CommandLineEntryPoint = "TheCat.EditorTools.P0DreamRouteBatch86RuntimeEvidenceBatchmodeRunner.RunForBatchmode";
        ...
        public static void RunForBatchmode()
        {
            try
            {
                ...
                hasStarted = true;
                Debug.Log("[TheCat] Batch 86 dream-route runtime evidence runner invoked.");
                ResetState();
                ...
                Debug.Log("[TheCat] Entering Play Mode for Batch 86 dream-route runtime evidence.");
        ...
        {
            Debug.Log("[TheCat] Batch 86 runtime evidence Play Mode state changed: " + change + ".");
        ...
        private static void Update()
        {
            ...
            try
            {
                ...
                if (!evidenceStarted)
                {
                    ...
                    {
                        ExitWithFailure("Timed out entering Play Mode before starting Batch 86 runtime evidence.");
                    }

                    return;
                }

                if (P0DreamRouteBatch86RuntimeEvidence.IsFinished)
                {
                    bool passed = P0DreamRouteBatch86RuntimeEvidence.State == P0DreamRouteBatch86RuntimeEvidenceState.Passed
                        && P0DreamRouteBatch86RuntimeEvidence.CapturedPaths.Count == P0DreamRouteBatch86RuntimeEvidence.ExpectedScreenshotCount
                        && P0DreamRouteBatch86RuntimeEvidence.GeneratedReviewPaths.Count == P0DreamRouteBatch86RuntimeEvidence.ExpectedAutomaticReviewCount;
                    CleanupAndExit(passed, P0DreamRouteBatch86RuntimeEvidence.LastSummary);
                    return;
                }

                if (elapsed > RuntimeEvidenceTimeoutSeconds)
                {
                    ExitWithFailure("Timed out waiting for Batch 86 dream-route runtime evidence to finish.");
                }
            }
            catch (Exception exception)
            {
        ...
        private static void StartEvidence()
        {
            ...
            Debug.Log("[TheCat] Starting Batch 86 dream-route runtime evidence.");
            P0DreamRouteBatch86RuntimeEvidence.SetBeforeCaptureResolutionOverride(P0DreamRouteBatch86GameViewSizeUtility.ApplyTargetSize);
            bool started = P0DreamRouteBatch86RuntimeEvidence.StartDefaultRuntimeEvidence();
            if (!started)
            {
                ExitWithFailure("Could not start Batch 86 runtime evidence: " + P0DreamRouteBatch86RuntimeEvidence.LastSummary);
            }
        }
        ...
        private static void CleanupAndExit(bool passed, string message)
        {
            ...
            string logMessage = "[TheCat] Batch 86 dream-route runtime evidence " + (passed ? "passed: " : "failed: ") + message;
            if (passed)
            {
        ...
        {
            string path = ToProjectPath(P0DreamRouteBatch86RuntimeEvidence.RuntimeReportPath);
            ...
            File.WriteAllText(
                ...
                P0DreamRouteBatch86RuntimeEvidence.BuildRuntimeReportMarkdown(
                    ...
                    message,
                    message,
                    P0DreamRouteBatch86RuntimeEvidence.CapturedPaths,
                    P0DreamRouteBatch86RuntimeEvidence.GeneratedReviewPaths));
        }
    ...
    internal static class P0DreamRouteBatch86GameViewSizeUtility
    {
        private const string CustomSizePrefix = "Batch86 ";

        public static string ApplyTargetSize(P0DreamRouteBatch86ScreenshotTarget target)
        {
            try
            {
