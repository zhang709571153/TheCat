...
{
    public static class P0ResultSettlementBatch84UnityPreflightRunner
    {
        private const string MenuPath = "TheCat/P0/Batch 84 Result Settlement Unity Preflight";

        [MenuItem(MenuPath, false, 92)]
        private static void RunFromMenu()
        {
            P0ResultSettlementBatch84UnityPreflightReport report = RunPreflight(writeReport: true);
            EditorUtility.DisplayDialog(
                report.IsReadyForUnityPreflight
                    ? "Batch 84 Result Settlement Preflight Ready"
                    : "Batch 84 Result Settlement Preflight Needs Attention",
                report.BuildSummary(),
                "OK");
        }
        ...
        public static P0ResultSettlementBatch84UnityPreflightReport RunPreflight(bool writeReport)
        {
            AssetDatabase.Refresh(ImportAssetOptions.ForceUpdate);

            P0AssetImportApplyReport applyReport = P0AssetImportSettingsApplier.Apply(
                P0ResultSettlementBatch84CandidateCatalog.CreateUnityPreflightManifest(),
        ...
        {
            string message = "[TheCat] Batch 84 import apply: " + report.BuildDetailedLog();
            if (report.IsApplied)
            {
                Debug.Log(message);
                return;
        ...
        {
            string message = "[TheCat] Batch 84 import validation: " + report.BuildDetailedLog();
            if (report.IsValid)
            {
                Debug.Log(message);
                return;
        ...
        private static void LogPreflightReport(P0ResultSettlementBatch84UnityPreflightReport report)
        {
            string message = "[TheCat] Batch 84 result/settlement preflight: " + report.BuildDetailedSummary();
        ...
        {
            string path = ToProjectPath(P0ResultSettlementBatch84UnityPreflight.ReportPath);
