...
{
    public static class P0CharacterSelectBatch88RuntimeEvidenceBatchmodeRunner
    {
        private const double EnterPlayModeTimeoutSeconds = 45d;
        private const double RuntimeEvidenceTimeoutSeconds = 160d;
        private const string CommandLineEntryPoint = "TheCat.EditorTools.P0CharacterSelectBatch88RuntimeEvidenceBatchmodeRunner.RunForBatchmode";
        ...
        public static void RunForBatchmode()
        {
            try
            {
                ...
                hasStarted = true;
                Debug.Log("[TheCat] Batch 88 character-select runtime evidence runner invoked.");
                ResetState();
                ...
                Debug.Log("[TheCat] Entering Play Mode for Batch 88 character-select runtime evidence.");
        ...
        {
            Debug.Log("[TheCat] Batch 88 runtime evidence Play Mode state changed: " + change + ".");
        ...
        private static void Update()
        {
            ...
            try
            {
                ...
                if (!evidenceStarted)
                {
                    ...
                    {
                        ExitWithFailure("Timed out entering Play Mode before starting Batch 88 runtime evidence.");
                    }

                    return;
                }

                if (P0CharacterSelectBatch88RuntimeEvidence.IsFinished)
                {
                    bool passed = P0CharacterSelectBatch88RuntimeEvidence.State == P0CharacterSelectBatch88RuntimeEvidenceState.Passed
                        && P0CharacterSelectBatch88RuntimeEvidence.CapturedPaths.Count == P0CharacterSelectBatch88RuntimeEvidence.ExpectedScreenshotCount
                        && P0CharacterSelectBatch88RuntimeEvidence.GeneratedReviewPaths.Count == P0CharacterSelectBatch88RuntimeEvidence.ExpectedAutomaticReviewCount;
                    CleanupAndExit(passed, P0CharacterSelectBatch88RuntimeEvidence.LastSummary);
                    return;
                }

                if (elapsed > RuntimeEvidenceTimeoutSeconds)
                {
                    ExitWithFailure("Timed out waiting for Batch 88 character-select runtime evidence to finish.");
                }
            }
            catch (Exception exception)
            {
        ...
        private static void StartEvidence()
        {
            ...
            Debug.Log("[TheCat] Starting Batch 88 character-select runtime evidence.");
            P0CharacterSelectBatch88RuntimeEvidence.SetBeforeCaptureResolutionOverride(P0CharacterSelectBatch88GameViewSizeUtility.ApplyTargetSize);
            bool started = P0CharacterSelectBatch88RuntimeEvidence.StartDefaultRuntimeEvidence();
            if (!started)
            {
                ExitWithFailure("Could not start Batch 88 runtime evidence: " + P0CharacterSelectBatch88RuntimeEvidence.LastSummary);
            }
        }
        ...
        private static void CleanupAndExit(bool passed, string message)
        {
            ...
            string logMessage = "[TheCat] Batch 88 character-select runtime evidence " + (passed ? "passed: " : "failed: ") + message;
            if (passed)
            {
        ...
        {
            string path = ToProjectPath(P0CharacterSelectBatch88RuntimeEvidence.RuntimeReportPath);
            ...
            File.WriteAllText(
                ...
                P0CharacterSelectBatch88RuntimeEvidence.BuildRuntimeReportMarkdown(
                    ...
                    message,
                    message,
                    P0CharacterSelectBatch88RuntimeEvidence.CapturedPaths,
                    P0CharacterSelectBatch88RuntimeEvidence.GeneratedReviewPaths));
        }
    ...
    internal static class P0CharacterSelectBatch88GameViewSizeUtility
    {
        private const string CustomSizePrefix = "Batch88 ";

        public static string ApplyTargetSize(P0CharacterSelectBatch88ScreenshotTarget target)
        {
            try
            {
