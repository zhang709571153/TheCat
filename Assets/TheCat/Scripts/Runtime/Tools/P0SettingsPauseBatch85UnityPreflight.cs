...
{
    public sealed class P0SettingsPauseBatch85UnityPreflightReport
    {
        ...
        private readonly List<P0SettingsPauseBatch85CandidateAsset> candidates = new List<P0SettingsPauseBatch85CandidateAsset>();
        ...
        public IReadOnlyList<P0SettingsPauseBatch85CandidateAsset> Candidates => candidates.AsReadOnly();
        ...
        public bool CandidatePreflightChecksReady => FailureCount == 0
            && coveredChecks.Count >= P0SettingsPauseBatch85UnityPreflight.ExpectedCoveredCheckCount;
        ...
        public void AddCandidate(P0SettingsPauseBatch85CandidateAsset candidate)
        {
        ...
        public string BuildSummary()
        {
            ...
            {
                return "Batch 85 settings/pause Unity preflight has " + FailureCount + " failure(s).";
            }

            if (!UnityEditorImportValidationReady)
            {
                return "Batch 85 settings/pause candidates need Unity editor import validation before preflight is ready.";
            }

            if (FormalInstallAllowed)
            {
                return "Batch 85 settings/pause candidates are Unity-import preflight ready and formal install evidence is complete.";
            }

            if (UnityEvidenceCount >= UnityEvidenceRequiredCount - 2)
            {
                return "Batch 85 settings/pause candidates are Unity-import preflight ready; runtime screenshots/reviews passed, formal install remains blocked by scene/Console and human approval gates.";
            }

            return "Batch 85 settings/pause candidates are Unity-import preflight ready; formal install remains blocked until Unity settings/pause screenshots, text replacement, 1024x768 key-hint semantics, click targets, scene/Console, and human approval gates pass.";
        }
        ...
        public string BuildMarkdown()
        {
            ...
            builder.AppendLine("# Batch 85 Settings Pause Unity Preflight");
            builder.AppendLine();
            ...
            builder.AppendLine("- Batch 85 imports are not included in `P0VisualAssetCatalog.P0RuntimeVisualBindingCount`.");
            builder.AppendLine("- Current pause/settings presenters remain authoritative until nonblank runtime screenshots, text replacement, key-hint semantics, click targets, Console, and human approval gates pass.");
            builder.AppendLine("- Screenshot evidence must be confirmed by `" + P0SettingsPauseBatch85UnityPreflight.RuntimeEvidenceReportPath + "`; scene/Console evidence must identify the matching clean runtime evidence log.");
            builder.AppendLine("- Do not mark Batch 85 as formally installed before explicit review approval.");
    ...
    public static class P0SettingsPauseBatch85UnityPreflight
    {
        public const int ExpectedCoveredCheckCount = 7;
        public const int ExpectedUnityEvidenceRequiredCount = 8;
        public const string ReportPath = "design/development/asset_review/BATCH85_SETTINGS_PAUSE_UNITY_PREFLIGHT_REPORT_2026-06-26.md";
        public const string RuntimeEvidenceReportPath = "design/development/asset_review/batch_85_settings_pause_unity_preflight/runtime_evidence_report.md";
        public const string EvidenceLogPrefix = "Batch 85 preflight log:";
        public const string PreflightPassedLogToken = "[TheCat] Batch 85 settings/pause preflight:";
        private const string RuntimeEvidenceLogPrefix = "Batch 85 runtime evidence log:";
        private const string RuntimeEvidencePassedLogToken = "[TheCat] Batch 85 settings/pause runtime evidence passed:";
        public const string CompleteScreenshotEvidenceToken = "Complete screenshot evidence: yes";
        public const string SettingsPauseSurfaceCapturedToken = "Settings/pause surface captured: yes";
        ...
        public const string SettingsMainAudioStatesVisibleToken = "Settings main/audio states visible: yes";
        public const string PauseOverlayStateVisibleToken = "Pause overlay state visible: yes";
        public const string KeyHintReadabilityVisibleToken = "Key-hint readability visible: yes";
        public const string SettingsPauseClickTargetsVisibleToken = "Settings/pause click targets visible: yes";
        ...
        public static readonly string[] RequiredUnityEvidencePaths =
        {
            ...
            "design/development/asset_review/batch_85_settings_pause_unity_preflight/scene_binding_console_clean_report.md",
            "design/development/asset_review/batch_85_settings_pause_unity_preflight/human_review_approval.md"
        };

        public static P0SettingsPauseBatch85UnityPreflightReport EvaluateCurrentPreflight()
        {
            return EvaluateCurrentPreflight(unityEditorImportValidationReady: false);
        }

        public static P0SettingsPauseBatch85UnityPreflightReport EvaluateCurrentPreflight(bool unityEditorImportValidationReady)
        {
            return Evaluate(
        ...
        public static P0SettingsPauseBatch85UnityPreflightReport Evaluate(
        ...
        {
            P0SettingsPauseBatch85UnityPreflightReport report = new P0SettingsPauseBatch85UnityPreflightReport();
            ...
            if (candidates != null)
            {
                for (int i = 0; i < candidates.Count; i++)
                {
                    ...
                    if (IsSettingsPauseUnityImportPath(entry.UnityImportPath) && exists(entry.UnityImportPath))
                    {
                    ...
                    {
                        report.AddIssue(entry.AssetId + " Unity preflight import is missing or outside SettingsPause: " + entry.UnityImportPath);
                    }
            ...
            int unityPreflightBindingCount = CountReadyBindings(P0SettingsPauseBatch85CandidateCatalog.CreateUnityPreflightBindings());
            ...
            Require(report, candidateCount == P0SettingsPauseBatch85CandidateCatalog.ExpectedCandidateSpriteCount, "Batch 85 catalog declares six settings/pause component candidates.", "Batch 85 candidate catalog count is wrong.");
            Require(report, sourceCandidatePngCount == candidateCount, "Batch 85 source candidate PNGs exist in design asset candidates.", "Batch 85 source candidate PNGs are incomplete.");
            Require(report, sourceCandidateNoMetaCount == candidateCount, "Batch 85 source candidate PNGs remain without Unity meta files.", "Batch 85 source candidates leaked Unity meta files.");
            Require(report, importedCandidatePngCount == candidateCount, "Batch 85 Unity preflight imports exist under SettingsPause.", "Batch 85 Unity preflight imports are incomplete.");
            Require(report, importedCandidateMetaCount == candidateCount, "Batch 85 Unity preflight imports have Unity meta files.", "Batch 85 Unity preflight import meta files are incomplete.");
            Require(report, dimensionMatchedCount == candidateCount, "Batch 85 Unity preflight imports match manifest dimensions.", "Batch 85 Unity preflight import dimensions do not match.");
            Require(report, unityPreflightBindingCount == candidateCount, "Batch 85 preflight-only bindings are complete and ready.", "Batch 85 preflight-only bindings are incomplete.");
            Require(report, formalRuntimeBindingLeakCount == 0, "Batch 85 candidates are isolated from formal runtime visual bindings.", "Batch 85 candidate assets leaked into formal runtime visual bindings.");
            Require(report, queueEntryReadyForUnityReview, "Batch 85 settings/pause queue entry is ready for Unity review.", "Batch 85 queue entry is missing or not ready for Unity review.");

            if (!unityEditorImportValidationReady)
            {
                report.AddBlockingItem("Unity editor import validation has not passed for Batch 85.");
            }
            ...
            for (int i = 0; i < runtimeBindings.Length; i++)
            {
                ...
                for (int j = 0; j < candidates.Count; j++)
                {
                    ...
                    if (binding.Asset.AssetId == entry.AssetId
                        ...
                        || binding.BindingId.IndexOf("batch85.", StringComparison.OrdinalIgnoreCase) >= 0)
                    {
                        leaks++;
                    }
                }
            ...
            P0SettingsPauseBatch85UnityPreflightReport report)
        {
            ...
            {
                report.AddBlockingItem("Unknown Batch 85 screenshot target dimensions for `" + path + "`.");
                return false;
            }

            if (!readDimensions(path, out int actualWidth, out int actualHeight, out string error))
            {
                report.AddBlockingItem("Unreadable Batch 85 screenshot evidence: `" + path + "` (" + error + ").");
                return false;
            }
            ...
            P0SettingsPauseBatch85UnityPreflightReport report)
        {
            ...
            {
                report.AddBlockingItem("Missing Batch 85 runtime evidence report for screenshot evidence: `" + RuntimeEvidenceReportPath + "`.");
                return false;
            }
            ...
            {
                report.AddBlockingItem("Unreadable Batch 85 runtime evidence report: `"
            ...
            string[] requiredTokens =
            {
                ...
                SettingsPauseSurfaceCapturedToken,
                CandidateFrameDrawToken,
                NoCandidateTextureFallbackToken,
                SettingsMainAudioStatesVisibleToken,
                PauseOverlayStateVisibleToken,
                KeyHintReadabilityVisibleToken,
                SettingsPauseClickTargetsVisibleToken
            };
            ...
            for (int i = 0; i < requiredTokens.Length; i++)
            {
                ...
                report.AddBlockingItem("Incomplete Unity screenshot evidence: `"
                    + path
                    + "` is not confirmed by a complete candidate-drawn Batch 85 runtime evidence report requiring `"
                    + requiredTokens[i]
                    + "`.");
                return false;
            }
            ...
            if (!ContainsOrdinal(content, path))
            {
                report.AddBlockingItem("Incomplete Unity screenshot evidence: `"
                    + path
                    + "` is not listed in the Batch 85 runtime evidence report.");
                return false;
            }
            ...
            P0SettingsPauseBatch85UnityPreflightReport report)
        {
            ...
            if (!ContainsOrdinalIgnoreCase(content, "Batch 85"))
            {
                report.AddBlockingItem("Incomplete Unity evidence: `" + path + "` does not name Batch 85.");
                return false;
            }
            ...
            if (path.EndsWith("text_replacement_key_hint_readability_review.md", StringComparison.Ordinal))
            {
                return RequireEvidenceTokens(
                    ...
                    path,
                    content,
                    new[] { "Review result: pass", "Unity-rendered text replacement: pass", "Key-hint semantics: pass", "1024x768 density: pass" });
            }
            ...
            if (path.EndsWith("settings_controls_click_target_review.md", StringComparison.Ordinal))
            {
                return RequireEvidenceTokens(
                    ...
                    path,
                    content,
                    new[] { "Review result: pass", "Slider/switch/checkbox controls: pass", "Tab and button click targets: pass", "Pause/resume/restart actions: pass" });
            }

            report.AddBlockingItem("Unknown Batch 85 markdown evidence path: `" + path + "`.");
            return false;
        }
        ...
        private static bool ValidateSceneBindingConsoleCleanReport(
            ...
            P0SettingsPauseBatch85UnityPreflightReport report)
        {
            string logPath = ExtractEvidenceLogPath(content, RuntimeEvidenceLogPrefix);
            if (string.IsNullOrWhiteSpace(logPath))
            {
                report.AddBlockingItem("Incomplete Unity evidence: `"
                    + path
                    + "` must include `"
                    + RuntimeEvidenceLogPrefix
                    + " <path>` for the clean runtime evidence log.");
                return false;
            }

            if (!exists(logPath))
            {
                report.AddBlockingItem("Console clean report references a missing Batch 85 runtime evidence log: `" + logPath + "`.");
                return false;
            }
            ...
            {
                report.AddBlockingItem("Unreadable Batch 85 runtime evidence log: `"
                    + logPath
                    + "` ("
                    + exception.GetType().Name
                    + ").");
            ...
            {
                report.AddBlockingItem("Console clean report log does not contain the Batch 85 runtime evidence pass token: `" + RuntimeEvidencePassedLogToken + "`.");
                return false;
            }
        ...
        private static bool IsQueueEntryReadyForUnityReview()
        {
            ...
            for (int i = 0; i < queue.Count; i++)
            {
                ...
                if (entry.QueueId == P0AssetProductionQueueCatalog.SettingsPausePreflightCandidateQueueId)
                {
                    return entry.State == P0AssetProductionQueueState.CandidatePackCompletePendingUnityReview
                        ...
                        && ContainsOrdinal(entry.CandidateDirectory, P0SettingsPauseBatch85CandidateCatalog.BatchSlug)
                        && ContainsOrdinal(entry.UnityImportRoot, "Assets/TheCat/Art/UI/SettingsPause")
                        && ContainsOrdinal(entry.NextAction, "Unity-rendered")
                        && ContainsOrdinal(entry.NextAction, "settings")
                        && ContainsOrdinal(entry.NextAction, "pause");
                }
            }
        ...
        private static bool IsSettingsPauseUnityImportPath(string path)
        {
            return ContainsOrdinal(path, "Assets/TheCat/Art/UI/SettingsPause/")
        ...
        private static string ExtractEvidenceLogPath(string content, string prefix)
        {
